...
class MyClass
{
	private $username = "enigma";
	private $password = "enigma";
	private $hostname = "192.168.12.13"; 
	private $dbname = "enigmaaiadb";
	private $dbhandle="";
  ...
  public function __construct()
  {
		...
		$selected = mysql_select_db($this->dbname,$this->dbhandle)
		or die("Could not select database\n");

		$this->tanggal = "'2014-09-19'";
  ...
  public function __getSalesList()
  {
		...
		foreach ($other as $key => $oth) {
			...
			$oth['tm_code']=$p['tm_code'];
			$oth['tsr_name']=$p['tsr_name'];	
			$oth['CampaignCode']=$p['CampaignCode'];		
		...
		foreach ($ben as $key => $bnf) {
			...
			$bnf['ProductCode']=$p['ProductCode'];
			$bnf['CampaignCode']=$p['CampaignCode'];
			$bnf['tm_code']=$p['tm_code'];
  ...
  public function _load_payer()
	{
		...
		$q="SELECT distinct
				...
				'' Address_4_Type_2,
				'' Postcode_Type_2,
				'' Address_State_Type_2,
				'' Address_Country_Type_2,
				...
				and c.CampaignId>=4
				and date(c.CustomerUpdatedTs)=".$this->tanggal;
	...
	public function _load_beneficiary()
	{
		...
		$q="SELECT 
			...
			and date(cus.CustomerUpdatedTs)=".$this->tanggal."
			";
	...
	public function CoverCode()
	{
		...
		$q="SELECT a.CustomerId, count(b.InsuredId) cn, sum(b.PremiumGroupId) sm, 
			...
			and a.CampaignId>=4
			and date(a.CustomerUpdatedTs)=".$this->tanggal."
