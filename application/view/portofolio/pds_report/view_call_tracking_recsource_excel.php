...
/* ------------------------------------------------------------------------------------ */
$arr_rows+=2;
foreach($LoopRecs as $ID => $rows){
	// agent title
	$arr_rows+=1;
	// table
	$worksheet->write_string($arr_rows, 0, "recsource : ".$rows, $agent_format );
	...
	foreach($RowData1[$ID] as $Data => $rows1){
		$Recsource		= ($LoopUser[$rows1['SellerId']]['id'] ? $LoopUser[$rows1['SellerId']]['id']."-".$LoopUser[$rows1['SellerId']]['fullname'] : 0);
		...
		$product		= ($rows1['product'] ? $rows1['product'] : '-');
		...
		$worksheet->write_number($arr_rows, 12, $B);
		$worksheet->write_number($arr_rows, 13, $TBO);
		$worksheet->write_number($arr_rows, 14, $INC);
		$worksheet->write_number($arr_rows, 15, $INC_UNVER);
		$worksheet->write_number($arr_rows, 16, $INC_NOTPASS);
		...
		$worksheet->write_string($arr_rows, $loopcol+=1, $product);
		$worksheet->write_number($arr_rows, $loopcol+=1, 0);
		$arr_rows+=1;
	...
	$worksheet->write_string($arr_rows, 0, "Totals", $header_format);
	$worksheet->write_number($arr_rows, 1, $sDataSize[$ID]['sDataSize'], $header_format);
	$worksheet->write_number($arr_rows, 2, $sUtilize[$ID]['sUtilize'], $header_format);
	$worksheet->write_number($arr_rows, 3, $sContactedInit[$ID]['sContacted'], $header_format);
	$worksheet->write_number($arr_rows, 4, round($sFreq_1[$ID]['sFreq_1'],1), $header_format);
	$worksheet->write_number($arr_rows, 5, $sUncontactInit[$ID]['sUncontact'], $header_format);
	$worksheet->write_number($arr_rows, 6, round($sFreq_2[$ID]['sFreq_2'],1), $header_format);
	$worksheet->write_number($arr_rows, 7, $sTotal[$ID]['sTotal'], $header_format);
	$worksheet->write_number($arr_rows, 8, round($sFreq_3[$ID]['sFreq_3'],1), $header_format);
	$worksheet->write_string($arr_rows, 9, round($sPerUtil[$ID]['sPerUtil'],2)." %", $header_format);
	$worksheet->write_number($arr_rows, 10, $sD[$ID]['sD'], $header_format);
	$worksheet->write_number($arr_rows, 11, $sST[$ID]['sST']+$sCB[$ID]['sCB'], $header_format);
	$worksheet->write_number($arr_rows, 12, $sB[$ID]['sB'], $header_format);
	$worksheet->write_number($arr_rows, 13, $sTBO[$ID]['sTBO'], $header_format);
	$worksheet->write_number($arr_rows, 14, $sINC[$ID]['sINC'], $header_format);
	$worksheet->write_number($arr_rows, 15, $sINC_UNVER[$ID]['sINC_UNVER'], $header_format);
	$worksheet->write_number($arr_rows, 16, $sINC_NOTPASS[$ID]['sINC_NOTPASS'], $header_format);
	$worksheet->write_number($arr_rows, 17, $sContacted[$ID]['sContacted'], $header_format);
	$worksheet->write_string($arr_rows, 18, round($sPerContact[$ID]['sPerContact'],2)." %", $header_format);
	$worksheet->write_number($arr_rows, 19, $sNP[$ID]['sNP'], $header_format);
	$worksheet->write_number($arr_rows, 20, $sBT[$ID]['sBT'], $header_format);
	$worksheet->write_number($arr_rows, 21, $sNA[$ID]['sNA'], $header_format);
	$worksheet->write_number($arr_rows, 22, $sMV[$ID]['sMV'], $header_format);
	$worksheet->write_number($arr_rows, 23, $sWN[$ID]['sWN'], $header_format);
	$worksheet->write_number($arr_rows, 24, $sID[$ID]['sID'], $header_format);
	$worksheet->write_number($arr_rows, 25, $sUncontact[$ID]['sUncontact'], $header_format);
	$worksheet->write_string($arr_rows, 26, round($sPerUncontact[$ID]['sPerUncontact'],2)." %", $header_format);
	$totcols = 26;
	foreach($LoopDL as $keys => $DLx){
		$worksheet->write_number($arr_rows, $totcols+=1, $sDL[$ID][$DLx], $header_format);
	}
	$worksheet->write_number($arr_rows, $totcols+=1, 0, $header_format);
	$worksheet->write_number($arr_rows, $totcols+=1, $sAmount[$ID]['sAmount'], $header_format);
	$worksheet->write_number($arr_rows, $totcols+=1, $sINC[$ID]['sINC'], $header_format);
