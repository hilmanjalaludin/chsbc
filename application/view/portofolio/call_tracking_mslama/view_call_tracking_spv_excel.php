<?php 
 ...
 $arr_title = "Call Tracking Summary by Recsource per SPV Periode of"._convertDate($param['start_date']) ."to". _convertDate($param['end_date']);
...
foreach($LoopUser as $ID => $rows){
	...
	foreach($RowData1[$ID] as $Data => $rows1){
		...
		$sTBO[$ID]['sTBO']				+= $TBO;
		$sINC[$ID]['sINC']				+= $INC;
		$sINC_UNVER[$ID]['sINC_UNVER']	+= $INC_UNVER;
		$sINC_NOTPASS[$ID]['sINC_NOTPASS']	+= $INC_NOTPASS;
