...
// ========================================= DIS NEW DB  SUMMARY/ MGR =========================================
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data,a.AssignAmgr as AssignAmgr,
...
// ==================== SOLICITED NEW / MGR  ===================================
 ...
 $arr_utilize_new = null;
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['AssignAmgr']] = explode(',', $rows['arr_utilize_new_id']);
...
if( is_array($arr_utilize_new) and count($arr_utilize_new) > 0 ) 
	foreach( $arr_utilize_new as $MgrId => $CustomerId)
{
	$UI->db->reset_select();
	...
	$UI->db->where("a.CallHistoryCallDate>='{$Out->get_value('start_date','StartDate')}'", "", false);
	$UI->db->where("a.CallHistoryCallDate<='{$Out->get_value('end_date','EndDate')}'", "", false);
	$UI->db->where_in("d.UserId", array_map('intval', array($MgrId)));
	...
	if( $rs->num_rows() > 0 AND $rows = $rs->result_first_assoc() ) {
	   $data[$MgrId]['data_old_utilize'] += $rows['tot_old_utilize'];
	   $data[$MgrId]['data_all_solicited'] += $rows['tot_old_utilize'];
	}
...
// ============================== DIS NEW DB  ==============================
 ...
 $arr_dis_new_data  = array();
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data,a.AssignSpv as AssignSpv,
	GROUP_CONCAT(DISTINCT a.CustomerId ) as arr_dis_new_data  ", FALSE);
 $UI->db->from("t_gn_assignment_log a force index(primary)");
 $UI->db->join("t_gn_customer b","a.CustomerId=b.CustomerId", "LEFT OUTER");
 $UI->db->where("a.CallReasonId=99", "", FALSE);
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$arr_dis_new_data[$rows['AssignSpv']] = explode(",", $rows['arr_dis_new_data']);
...
// ===================== OLD DIS DATA ==============================================
 ...
 if(is_array( $arr_dis_new_data ) and count( $arr_dis_new_data) > 0 )
	 foreach( $arr_dis_new_data as $AssignATM => $CustomerId )
 {
	 ...
	 $UI->db->where_in("b.CampaignId", $CampaignId); 
	 $UI->db->where_in("a.AssignAmgr", array_map('intval', array($ManagerId) ));
	 $UI->db->where_in("a.AssignSpv", array_map('intval',  array($AssignATM)));
	 $UI->db->where("a.AssignDate>='{$obj->get_value('start_date','StartDate')}'", "", false);
	 $UI->db->where("a.AssignDate<='{$obj->get_value('end_date','EndDate')}'", "", false);
	 ...
	 if( $rs->num_rows() > 0 AND $rows = $rs->result_first_assoc() ) 
	{
		$data[$AssignATM]['tot_re_assigned'] += $rows['tot_re_assigned'];
	} 	 
 }
...
// ==================== SOLICITED NEW / ATM / MGR  ===================================
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['AssignSpv']] = explode(',', $rows['arr_utilize_new_id']);
...
if( is_array($arr_utilize_new) and count($arr_utilize_new) > 0 ) 
	foreach( $arr_utilize_new as $AssgAtmId => $CustomerId)
{
	$UI->db->reset_select();
	$UI->db->select("count(a.CustomerId) as tot_old_utilize", FALSE);
	$UI->db->from("t_gn_callhistory a  ");
	...
	$UI->db->where("a.CallBeforeReasonId<>99", "", FALSE);
	$UI->db->where_in("b.CampaignId", $CampaignId);
	$UI->db->where_in("c.UserId", array_map('intval', array($AssgAtmId)));
	$UI->db->where_in("d.UserId", array_map('intval', array($ManagerId)));
	$UI->db->where("a.CallHistoryCallDate>='{$obj->get_value('start_date','StartDate')}'", "", false);
	$UI->db->where("a.CallHistoryCallDate<='{$obj->get_value('end_date','EndDate')}'", "", false);
	...
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 AND $rows = $rs->result_first_assoc() ) {
	   $data[$AssgAtmId]['data_old_utilize'] += $rows['tot_old_utilize'];
	   $data[$AssgAtmId]['data_all_solicited'] += $rows['tot_old_utilize'];
	}
