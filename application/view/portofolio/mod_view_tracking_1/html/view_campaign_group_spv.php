 ...
 function _select_campaign_group_spv_summary()
{
	...
	$arr_var_spvs = $out->get_array_value('spvId');
...
// --- get data init freq_call per lead 
	...
	$sql = sprintf("SELECT
			...
			SUM( IF(( b.CallReasonCode IN('ST', 'CB', 'SA','SAP', 'PU','INC', 'GPU', 'CPGP', 'CPGP','R','B') OR b.CallCategoryCode IN('600') ), 1,0)) as tot_freq_call_contacted,
...
// --- get agent data ------------------------------------
  ...
  if( is_array($arr_var_spvs)  ) 
	foreach( $arr_var_spvs as $key=> $SpvId)
  {
 	
	$row_val = _select_row_user( $SpvId ) ;
...
// -- content -header --- 	
	...
	$agent_val_data = ( $arr_val_data[$SpvId] ? $arr_val_data[$SpvId] : null );
 ...
 function _showheaders()
{
	...
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Call Tracking - Summary By Recsource Per SPV</p>".
