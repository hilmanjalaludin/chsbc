 ...
 */
 ...
 function _select_report_group_recsource_summary()
{
	$CI  =& get_instance();
	$out  = _find_all_object_request();
	
	$arr_rec_value = _select_report_recsource_value();
	$arr_var_recsources = _select_report_recsource_id();
...
// --- get data init freq_call per lead 
	...
	$sql = sprintf("SELECT
			...
			SUM( IF(( b.CallReasonCode IN('ST', 'CB', 'SA','SAP','PU', 'GPU', 'CPGP','INC', 'CPGP','R','B') OR b.CallCategoryCode IN('600') ), 1,0)) as tot_freq_call_contacted,
			SUM(IF( b.CallReasonCode IN('NP', 'BT','NA', 'MV', 'WN', 'ID'), 1,0)) as tot_freq_call_uncontacted,
			c.AssignLeader as agent_id,
...
//printf(" init call : <pre>%s</pre><hr>", $sql);	
	...
	{
		$arr_val_data[$row['Recsource']]['tot_freq_init_call'] += $row['tot_init_call'];	
		$arr_val_data[$row['Recsource']]['tot_freq_call_contacted'] += $row['tot_freq_call_contacted'];	
		$arr_val_data[$row['Recsource']]['tot_freq_call_uncontacted'] += $row['tot_freq_call_uncontacted'];	
	}
...
// --------- get all POD ---------------------------------
	...
	if( $qry->num_rows() > 0) 
	 foreach( $qry->result_assoc() as $row )  {
		$arr_val_data[$row['Recsource']]['tot_pod'] += $row['tot_pod'];
	}
...
//	printf("ADDON : <pre>%s</pre><hr>", $sql);		
	...
	if( $qry->num_rows() > 0) 
	 foreach( $qry->result_assoc() as $row )  {
		$arr_val_data[$row['Recsource']]['tot_addon'] += $row['tot_addon'];
	}
...
//printf("Data Size : <pre>%s</pre><hr>", $sql);
	 ...
	 if( $qry->num_rows() > 0 )
		 foreach( $qry->result_assoc() as $row )
	{
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_data_size'];
	}
...
// --- get data utilize per status 	
	...
	$sql = sprintf("SELECT 
			...
			SUM(IF( b.CallReasonCode IN('SA','SAP'), 1, 0)) tot_sa,
...
//printf(" Data Util : <pre>%s</pre><hr>", $sql);
		 ...
		 foreach( $qry->result_assoc() as $row )
	{
		...
		$arr_val_data[$row['Recsource']]['data_util'] += $row['tot_utilize'];
		
	// ------------ contacted call -------------------------------------
	
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_dl'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_st'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_cb'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_sa'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_pu'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_gpu'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_cpgp'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_inc'];
		$arr_val_data[$row['Recsource']]['tot_contacted'] += $row['tot_rjct'];
		
		$arr_val_data[$row['Recsource']]['tot_dl'] += $row['tot_dl'];
		$arr_val_data[$row['Recsource']]['tot_st'] += $row['tot_st'];
		$arr_val_data[$row['Recsource']]['tot_cb'] += $row['tot_cb'];
		$arr_val_data[$row['Recsource']]['tot_sa'] += $row['tot_sa'];
		$arr_val_data[$row['Recsource']]['tot_pu'] += $row['tot_pu'];
		$arr_val_data[$row['Recsource']]['tot_gpu'] += $row['tot_gpu'];
		$arr_val_data[$row['Recsource']]['tot_cpgp']+= $row['tot_cpgp'];
		$arr_val_data[$row['Recsource']]['tot_inc'] += $row['tot_inc'];
		$arr_val_data[$row['Recsource']]['tot_rjct']+= $row['tot_rjct'];
		$arr_val_data[$row['Recsource']]['tot_blck']+= $row['tot_blck'];
		
		
	// ----------- un contact -------------------------------------------------------------
	
		$arr_val_data[$row['Recsource']]['tot_uncontacted'] += $row['tot_np'];
		$arr_val_data[$row['Recsource']]['tot_uncontacted'] += $row['tot_bt'];
		$arr_val_data[$row['Recsource']]['tot_uncontacted'] += $row['tot_na'];
		$arr_val_data[$row['Recsource']]['tot_uncontacted'] += $row['tot_mv'];
		$arr_val_data[$row['Recsource']]['tot_uncontacted'] += $row['tot_wn'];
		$arr_val_data[$row['Recsource']]['tot_uncontacted'] += $row['tot_id'];
		
		$arr_val_data[$row['Recsource']]['tot_np'] += $row['tot_np'];
		$arr_val_data[$row['Recsource']]['tot_bt'] += $row['tot_bt'];
		$arr_val_data[$row['Recsource']]['tot_na'] += $row['tot_na'];
		$arr_val_data[$row['Recsource']]['tot_mv'] += $row['tot_mv'];
		$arr_val_data[$row['Recsource']]['tot_wn'] += $row['tot_wn'];
		$arr_val_data[$row['Recsource']]['tot_id'] += $row['tot_id'];
		
		
	// ---------- total call init --- 	
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl0'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl1'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl2'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl3'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl4'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl5'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl6'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl7'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl8'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl9'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl10'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl11'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl12'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl13'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl14'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl15'];
		
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl16'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl17'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl18'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl19'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl20'];
		$arr_val_data[$row['Recsource']]['data_size'] += $row['tot_dl21'];
...
// --- get agent data ------------------------------------
  ...
  if( is_array($arr_var_recsources)  ) 
	foreach( $arr_var_recsources as $rec_name => $rec_val)
  {
	$rec_val_data = ( $arr_val_data[$rec_val] ? $arr_val_data[$rec_val] : null );
	
	$totDataSize = $rec_val_data['data_size'];
	$totDataUtil = $rec_val_data['data_util'];
	$totDataContacted = $rec_val_data['tot_contacted'];
	$totDataUnContacted = $rec_val_data['tot_uncontacted'];
	$totDataFreqInitCall = $rec_val_data['tot_freq_init_call'];
	$totDataFreqContactedInitCall = $rec_val_data['tot_freq_call_contacted'];
	$totDataFreqUnContactedInitCall = $rec_val_data['tot_freq_call_uncontacted'];
	$totDataADDON = $rec_val_data['tot_addon'];
	$totDataPOD = $rec_val_data['tot_pod'];
		
		
		// ----- data contacted -----------------------
	
		$DataDL	 	= $rec_val_data['tot_dl'];
		$DataSTH 	= $rec_val_data['tot_st'];
		$DataCB  	= $rec_val_data['tot_cb'];
		$DataSA  	= $rec_val_data['tot_sa'];
		$DataPU  	= $rec_val_data['tot_pu'];
		$DataGPU  	= $rec_val_data['tot_gpu'];
		$DataCPGP   = $rec_val_data['tot_cpgp'];
		$DataINC  	= $rec_val_data['tot_inc'];
		$DataRJCT  	= $rec_val_data['tot_rjct'];
		$DataBLC  	= $rec_val_data['tot_blck'];
		
		// --- data uncontacted ----------------------- 
			
		$DataNP	 	= $rec_val_data['tot_np'];
		$DataBT 	= $rec_val_data['tot_bt'];
		$DataNA  	= $rec_val_data['tot_na'];
		$DataMV  	= $rec_val_data['tot_mv'];
		$DataWN  	= $rec_val_data['tot_wn'];
		$DataID  	= $rec_val_data['tot_id'];
		
		$DataDL0	= $rec_val_data['tot_dl0'];
		$DataDL1	= $rec_val_data['tot_dl1'];
		$DataDL2	= $rec_val_data['tot_dl2'];
		$DataDL3	= $rec_val_data['tot_dl3'];
		$DataDL4	= $rec_val_data['tot_dl4'];
		$DataDL5	= $rec_val_data['tot_dl5'];
		$DataDL6	= $rec_val_data['tot_dl6'];
		$DataDL7	= $rec_val_data['tot_dl7'];
		$DataDL8	= $rec_val_data['tot_dl8'];
		$DataDL9	= $rec_val_data['tot_dl9'];
		$DataDL10	= $rec_val_data['tot_dl10'];
		$DataDL11	= $rec_val_data['tot_dl11'];
		$DataDL12	= $rec_val_data['tot_dl12'];
		$DataDL13	= $rec_val_data['tot_dl13'];
		$DataDL14	= $rec_val_data['tot_dl14'];
		$DataDL15	= $rec_val_data['tot_dl15'];
		$DataDL16	= $rec_val_data['tot_dl16'];
		$DataDL17	= $rec_val_data['tot_dl17'];
		$DataDL18	= $rec_val_data['tot_dl18'];
		$DataDL19	= $rec_val_data['tot_dl19'];
		$DataDL20	= $rec_val_data['tot_dl20'];
		$DataDL21	= $rec_val_data['tot_dl21'];
	   ...
	   // --------------- break ------------------------------------------------------
		...
		printf("%s", "<tr class=\"content\">");
			printf("<td class=\"content\" nowrap>%s</td>", $rec_val);
			...
			printf("<td class=\"content\" align=\"right\">%s</td>", eval_number($rec_val_data['tot_np']));
			printf("<td class=\"content\" align=\"right\">%s</td>", eval_number($rec_val_data['tot_bt']));
			printf("<td class=\"content\" align=\"right\">%s</td>", eval_number($rec_val_data['tot_na']));
			printf("<td class=\"content\" align=\"right\">%s</td>", eval_number($rec_val_data['tot_mv']));
			printf("<td class=\"content\" align=\"right\">%s</td>", eval_number($rec_val_data['tot_wn']));
			printf("<td class=\"content\" align=\"right\">%s</td>", eval_number($rec_val_data['tot_id']));
			...
			printf("<td class=\"content\" align=\"right\" nowrap>%s</td>", eval_number($rec_val_data['tot_addon']));
			
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL0));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL1));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL2));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL3));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL4));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL5));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL6));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL7));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL8));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL9));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL10));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL11));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL12));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL13));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL14));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL15));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL16));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL17));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL18));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL19));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL20));
			printf('<td class=\'content\' align=\'right\'>%s</td>', eval_number($DataDL21));


			
			printf("<td class=\"content\" align=\"right\">%s</td>", "");
			printf("<td class=\"content\" align=\"right\">%s</td>","");
			printf("<td class=\"content\" align=\"right\">%s</td>", eval_number($rec_val_data['tot_pod']));
		printf("%s", "</tr>");
 ...
 function _showReport()
{
	if( _get_post('interval') =='summary' ){
		_select_report_group_recsource_summary();		
	}
 ...
 function _showheaders()
{
	...
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Call Tracking - Summary By Recsource</p>".
