 ...
 function _select_campaign_group_mgr_summary()
{
	...
	$arr_var_mgrs = $out->get_array_value('ManagerId');
...
// --- get data init freq_call per lead 
	...
	$sql = sprintf("SELECT
			...
			SUM( IF(( b.CallReasonCode IN('ST', 'CB', 'SA','SAP','PU', 'GPU','INC', 'CPGP', 'CPGP','R','B') OR b.CallCategoryCode IN('600') ), 1,0)) as tot_freq_call_contacted,
			SUM(IF( b.CallReasonCode IN('NP', 'BT','NA', 'MV', 'WN', 'ID'), 1,0)) as tot_freq_call_uncontacted,
			c.AssignAmgr as agent_id,
...
// --- get agent data ------------------------------------
  ...
  if( is_array($arr_var_mgrs)  ) 
	foreach( $arr_var_mgrs as $key=> $MgrId)
  {
 	
	$row_val = _select_row_user( $MgrId ) ;
...
// -- content -header --- 	
	...
	$agent_val_data = ( $arr_val_data[$MgrId] ? $arr_val_data[$MgrId] : null );
 ...
 function _showheaders()
{
	...
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Call Tracking - Summary By Recsource Per AM</p>".
