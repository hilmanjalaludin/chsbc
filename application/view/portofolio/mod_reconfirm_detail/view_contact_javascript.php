	...
	else 
	{
		...
		{ */
			...
			{ */
			  ...
			  Ext.Ajax
			  ({
				 url 	: Ext.DOM.INDEX +'/ModSaveActivity/SaveActivityConfirm/',
				 method : 'POST',
...
// data keluarkan alert.
	...
	$('#tabs_second-7').load( protectedAjaxUrlSecond.Apply(), {	CustomerId 	: CustomerId, Mode : 'INPUT' }, 
		function( response, status, xhr ){
			console.log( xhr.status );
		});
}
...
Ext.DOM.HandleDisagreeX = function()
{
	var QualityStatus = Ext.Cmp('QualityStatus').getValue();
	// alert(QualityStatus);
	if(QualityStatus=='5')
	{
		Ext.Cmp('CallDisagree').disabled(false);
