...
var reason = [];
var AgentScript = { };
var firstProd = false, secondProd = false;
...
// data keluarkan alert.
	...
	var protectedAjaxUrlSecond = Ext.EventUrl( new Array(Controller,'index') );
	$('#tabs_second-2').load( protectedAjaxUrlSecond.Apply(), { CustomerId : CustomerId, Mode : 'INPUT' },
...
Ext.DOM.LoadCdd = function()
{
	...
	if(SecondProduct){
		if( Controller_2nd == ''){
			...
			var protectedAjaxUrlSecond = Ext.EventUrl( new Array(Controller_2nd,'index') );
...
var CustomerId = Ext.Cmp('CustomerId').getValue(),
	...
	var CustomerId_2nd = Ext.Cmp('CustomerId_2nd').getValue(),
		...
		GenderId_2nd = Ext.Cmp('GenderId').getValue();
...
//   if(<?php echo $ver_res_stat?> == 0){
// 	  console.log('masuk',0)
// 	$("#tabs").mytab().tabs({ selected : 0 ,disabled:[2,3]});
//   }else if (<?php echo $ver_res_stat?> ==1){
// 	console.log('masuk',1)
// 	//PR BUAT RANGGA
