...
// ==================== SOLICITED NEW / MGR  ===================================
 ...
 $arr_utilize_new = null;
  ...
  $UI->db->group_by(array('tgl','AssignAmgr','AssignSellerId'));
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['tgl']][$rows['AssignAmgr']][$rows['AssignSellerId']]= explode(',', $rows['arr_utilize_new_id']);
...
// ==================== SOLICITED OLD / MGR  ===================================
	...
	$UI->db->group_by(array("tgl","AssignAmgr", "AgentId","CustomerId"));
	
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$arr_skip_data = $arr_utilize_new[$rows['tgl']][$rows['AssignAmgr']][$rows['AgentId']];
...
// ==================== SOLICITED NEW / ATM / MGR  ===================================
  ...
  $UI->db->select("
			date(a.CallHistoryCallDate) as tgl,
			a.ATMID as AssignSpv, a.AgentId as AssignSellerId,
			COUNT(a.CustomerId) as tot_utilize_new,
			GROUP_CONCAT( distinct a.CustomerId) as arr_utilize_new_id ", FALSE);
  $UI->db->from("t_gn_callhistory_newutil a ");
  $UI->db->where_in("a.CampaignId", $CampaignId);
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['tgl']][$rows['AssignSpv']][$rows['AssignSellerId']] = explode(',', $rows['arr_utilize_new_id']);
...
// ==================== SOLICITED OLD /ATM/MGR  ===================================
	...
	$UI->db->where_in("e.UserId", array_map('intval', array($ManagerId)));
 	$UI->db->group_by(array("tgl","AssignSpv", "AgentId","CustomerId"));
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$arr_skip_data = $arr_utilize_new[$rows['tgl']][$rows['AssignSpv']][$rows['AgentId']];
		if( !in_array($rows['CustomerId'], $arr_skip_data)) {
