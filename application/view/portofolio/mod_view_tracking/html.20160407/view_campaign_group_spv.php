...
// ===================== SIZE DIS NEW DATA ============================
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data,a.AssignLeader as AssignLeader,
...
{
	$arr_dis_new_data = explode(',', $rows['arr_dis_new_data']);
...
// ===================== SIZE DIS OLD DATA ============================
 ...
 if( count($arr_dis_new_data) > 0 ){
	$UI->db->where_not_in("a.CustomerId", array_map('intval', $arr_dis_new_data) );
 } 
...
$arr_utilize_new = null;
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['AssignLeader']] = explode(',', $rows['arr_utilize_new_id']);
...
if( is_array($arr_utilize_new) and count($arr_utilize_new) > 0 ) 
	foreach( $arr_utilize_new as $AssignLeader => $CustomerId)
{
	$UI->db->reset_select();
	$UI->db->select("count(a.CustomerId) as tot_old_utilize", FALSE);
	$UI->db->from("t_gn_callhistory a  ");
	$UI->db->join("t_gn_customer b "," a.CustomerId= b.CustomerId", "LEFT OUTER");
	$UI->db->join("tms_agent c "," a.ATMCode= c.id", "LEFT OUTER");
	$UI->db->join("tms_agent d "," a.AMGRCode= d.id", "LEFT OUTER");
	$UI->db->join("tms_agent e "," a.SPVCode= e.id", "LEFT OUTER");
	$UI->db->where("a.HistoryType", 0);
	$UI->db->where("a.CallBeforeReasonId<>99", "", FALSE);
	$UI->db->where_in("b.CampaignId", $CampaignId);
	$UI->db->where("a.CallHistoryCallDate>='{$Out->get_value('start_date','StartDate')}'", "", false);
	$UI->db->where("a.CallHistoryCallDate<='{$Out->get_value('end_date','EndDate')}'", "", false);
	$UI->db->where_in("e.UserId", array_map('intval',  array($AssignLeader) ));
	$UI->db->where_in("c.UserId", array_map('intval', $Out->get_array_value('AtmId')));
	$UI->db->where_in("d.UserId", array_map('intval', $Out->get_array_value('ManagerId')));
	
	if( count($CustomerId) == 0 ){ 
		$CustomerId = array('DEFAULT'); 
	}
	
	$UI->db->where_not_in("a.CustomerId", array_map('strval', $CustomerId));
	
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 AND $rows = $rs->result_first_assoc() ) {
	   $data[$AssignLeader]['data_old_utilize'] += $rows['tot_old_utilize'];
	   $data[$AssignLeader]['data_all_solicited'] += $rows['tot_old_utilize'];
	}
...
// ===================== OLD DIS DATA ==============================================
 ...
 if(is_array( $arr_dis_new_data ) and count( $arr_dis_new_data) > 0 )
	 foreach( $arr_dis_new_data as $AssignSelerId => $CustomerId )
 {
	 $UI->db->reset_select();
	 $UI->db->select("count( distinct a.CustomerId) as tot_re_assigned", FALSE);
	 $UI->db->from("t_gn_assignment_log a ");
	 $UI->db->join("t_gn_customer b","a.CustomerId=b.CustomerId", "LEFT OUTER");
	 $UI->db->where("a.CallReasonId<>99", "", FALSE);
	 $UI->db->where_in("b.CampaignId", $CampaignId); 
	 ...
	 $UI->db->where_in("a.AssignSpv", array_map('intval', $obj->get_array_value('AtmId')));
	 $UI->db->where_in("a.AssignLeader",array($spvId));
	 $UI->db->where_in("a.AssignSelerId", array_map('intval', array($AssignSelerId)));
	 
	 if( count($CustomerId) > 0 ){
		$UI->db->where_not_in("a.CustomerId", array_map('intval', $CustomerId));
	 }
	...
	// $UI->db->print_out();
	 ...
	 if( $rs->num_rows() > 0 AND $rows = $rs->result_first_assoc() ) 
	{
		$data[$AssignSelerId]['tot_re_assigned'] += $rows['tot_re_assigned'];
	} 	 
 }
...
// ==================== SOLICITED NEW / TMR  ===================================
 ...
 // $UI->db->print_out();
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['AssignSelerId']] = explode(',', $rows['arr_utilize_new_id']);
...
if( is_array($arr_utilize_new) and count($arr_utilize_new) > 0 ) 
	foreach( $arr_utilize_new as $AssignSelerId => $CustomerId)
{
	$UI->db->reset_select();
	$UI->db->select("count(a.CustomerId) as tot_old_utilize", FALSE);
	$UI->db->from("t_gn_callhistory a  ");
	...
	$UI->db->where("a.CallHistoryCallDate>='{$obj->get_value('start_date','StartDate')}'", "", false);
	$UI->db->where("a.CallHistoryCallDate<='{$obj->get_value('end_date','EndDate')}'", "", false);
	$UI->db->where_in("a.CreatedById", array_map('intval', array($AssignSelerId)));
	$UI->db->where_in("e.UserId", array_map('intval',  array($spvId) ));
	$UI->db->where_in("c.UserId", array_map('intval', $obj->get_array_value('AtmId')));
	$UI->db->where_in("d.UserId", array_map('intval', $obj->get_array_value('ManagerId')));
	...
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 AND $rows = $rs->result_first_assoc() ) {
	   $data[$AssignSelerId]['data_old_utilize'] += $rows['tot_old_utilize'];
	   $data[$AssignSelerId]['data_all_solicited'] += $rows['tot_old_utilize'];
	}
