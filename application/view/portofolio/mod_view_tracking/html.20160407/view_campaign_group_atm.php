...
// ===================== DIS NEW  DB  =====================
 ...
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data,a.AssignSpv as AssignSpv,
	group_concat(distinct a.CustomerId) as ar_dis_new_db,  ", FALSE);
...
// ===================== DIS OLD DB  =====================
 ...
 if( count($ar_dis_new_db) > 0 ){
	$UI->db->where_not_in("a.CustomerId", $ar_dis_new_db);
 }
 //$UI->db->print_out();
...
// ==================== SOLICITED NEW ===================================
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new_id = explode(',', $rows['arr_utilize_new_id']); 
 ...
 $UI->db->reset_select();
 $UI->db->select("count(a.CustomerId) as tot_old_utilize, c.UserId as AssignSpv", FALSE);
 ...
 if( count($arr_utilize_new_id) > 0 ){
	$UI->db->where_not_in("a.CustomerId", $arr_utilize_new_id);
 }
 
 $UI->db->group_by("AssignSpv");
 
 $rs = $UI->db->get();
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignSpv']]['data_new_utilize'] += $rows['tot_old_utilize'];
...
// $UI->db->print_out();
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$arr_dis_new_data[$rows['AssignLeader']]= explode(',', $rows['arr_dis_new_data']);
...
if(is_array($arr_dis_new_data) )
	foreach( $arr_dis_new_data as $AsignLeader => $SkipData )
{
	$UI->db->reset_select();
	...
	$UI->db->where_in("a.AssignAmgr", array_map('intval', $obj->get_array_value('ManagerId')));
	$UI->db->where_in("a.AssignSpv", array_map('intval',  array($AtmId) ));
	$UI->db->where_in("a.AssignLeader", $AsignLeader);
	$UI->db->where("a.AssignDate>='{$obj->get_value('start_date','StartDate')}'", "", false);
	$UI->db->where("a.AssignDate<='{$obj->get_value('end_date','EndDate')}'", "", false);
	
	if( count($SkipData) > 0 ){
		$UI->db->where_not_in("a.CustomerId", $SkipData);
	}
	
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 AND $row = $rs->result_first_assoc() )  {
		$data[$AsignLeader]['tot_re_assigned']+= $rows['tot_re_assigned'];
	} 
}		
...
// ==================== SOLICITED NEW / LEADER  ===================================
 ...
 if(is_array($arr_utilize_new) )
	foreach( $arr_utilize_new as $AsignLeader => $SkipData )
{
	$UI->db->reset_select();
	...
	$UI->db->where("a.CallHistoryCallDate>='{$obj->get_value('start_date','StartDate')}'", "", false);
	$UI->db->where("a.CallHistoryCallDate<='{$obj->get_value('end_date','EndDate')}'", "", false);
	$UI->db->where_in("e.UserId", array_map('intval', array($AsignLeader)));
	...
	$UI->db->where_not_in("a.CustomerId", array_map('strval', $SkipData));
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 AND $rows = $rs->result_first_assoc() ) 
	{
	   $data[$AsignLeader]['data_old_utilize'] += $rows['tot_old_utilize'];
	   $data[$AsignLeader]['data_all_solicited'] += $rows['tot_old_utilize'];
	}
}
