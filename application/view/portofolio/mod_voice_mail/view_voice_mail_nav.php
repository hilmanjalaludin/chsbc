...
var navigation = {
	custnav : Ext.DOM.INDEX +'/VoiceMail/index/',
	custlist : Ext.DOM.INDEX +'/VoiceMail/content/'
}
...
$(function(){
	...
	({
		...
		extTitle  : [['Search'],['Clear'],['Download'],[]],
		extMenu   : [['searchCustomer'],['Clear'],['Download'],[]],
		extIcon   : [['zoom.png'], ['cancel.png'],['disk.png'],[]],
		extText   : true,
		extInput  : true,
		extOption : [{
			render : 3,
...
Ext.DOM.VoiceDetail = function(object) {
	if(object.checked) 
	{ 
		Ext.EQuery.Ajax ({
			url : Ext.DOM.INDEX +"/VoiceMail/VoiceDetail/",
			param : {
				VoiceMailId : object.value
			}
		});
	}
}
...
<div id="span_top_nav">
	...
	<div id="result_content_add" class="box-shadow" style="padding-bottom:4px;margin-top:2px;margin-bottom:8px;">
		...
		<tr>
			<td class="text_caption"> From Number</td>
			<td><?php echo form()->input('anumber','input_text long',_get_post('agent_ext')); ?></td>
		</tr>
		
		<tr>
			<td class="text_caption"> Destination Number</td>
			<td><?php echo form()->input('bnumber','select long',$combo['AgentId'],_get_post('agent_id')); ?></td>
		</tr>
		
		<tr>
			<td class="text_caption"> Interval </td>
			<td><?php echo form()->input('start_time','input_text box',_get_post('start_time')); ?> - 
				<?php echo form()->input('end_time','input_text box',_get_post('end_time')); ?> 
			</td>
		</tr>
