 ...
 function _select_campaign_group_spv_summary()
{
 ...
 $TmrId = array_keys($cls->_select_report_tmr_by_spv($Out->get_value("spvId")));
...
// ===================== SIZE DIS NEW DATA ============================
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data,
 a.AssignLeader as AssignLeader,a.AssignSelerId as AssignSelerId,
 ...
 $UI->db->group_by(array("AssignLeader","AssignSelerId"));
...
// ===================== SIZE DIS OLD DATA ============================
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_re_assigned,a.AssignLeader as AssignLeader,
...
// ==================== SOLICITED NEW / LEADER  ===================================
  ...
  $UI->db->reset_select();
  $UI->db->select("
			a.SPVID as AssignLeader, a.AgentId as AgentId,
  ...
  $UI->db->group_by(array('AssignLeader','AgentId'));
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['AgentId']] = explode(',', $rows['arr_utilize_new_id']);
...
// ==================== SOLICITED OLD / LEADER  ===================================
 ...
 // if( is_array( $arr_utilize_new ) ) 
	 // foreach( $arr_utilize_new as $AgentId => $SkipData )
// {
	$UI->db->reset_select();
    $UI->db->select("
		count(distinct a.CustomerId) as tot_old_utilize, 
		c.UserId as AssignSpv,
	...
	$UI->db->where_in("e.UserId", array_map('intval', $Out->get_array_value('ManagerId')));
 	$UI->db->group_by(array("AgentId","CustomerId"));
	
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$arr_skip_data = $arr_utilize_new[$rows['AgentId']];
...
// ================ GET DATA ATEMPT PER ATM SUMMARY 1=======================================
...
FALSE);
 ...
 $UI->db->where_not_in("f.CallReasonCode", array('601','602','603'));
 $UI->db->where_in("b.CampaignId", $CampaignId );
 $UI->db->where("a.HistoryType", 0);
 $UI->db->where("a.CallHistoryCallDate>='{$Out->get_value('start_date','StartDate')}'", "", false);
 $UI->db->where("a.CallHistoryCallDate<='{$Out->get_value('end_date','EndDate')}'", "", false);
 ...
 $UI->db->where_in("d.UserId", array_map('intval', $Out->get_array_value('AtmId')));
 $UI->db->where_in("e.UserId", array_map('intval', $Out->get_array_value('ManagerId')));
 $UI->db->group_by(array('AssignLeader','a.CustomerId'));
...
//echo $UI->db->print_out();
 ...
 $UI->db->select("
	c.UserId as AssignLeader,
	SUM(DISTINCT IF(f.CallReasonCode IN ('601'),1,0)) as status_601,
	SUM(DISTINCT IF(f.CallReasonCode IN ('602'),1,0)) as status_602,
	SUM(DISTINCT IF(f.CallReasonCode IN ('603'),1,0)) as status_603", FALSE);
 ...
 $UI->db->where_not_in("f.CallReasonCode", $skip_status);
 $UI->db->where_in("f.CallReasonCode", array('601','602','603'));
 $UI->db->where_in("b.CampaignId", $CampaignId );
 $UI->db->where("a.HistoryType", 0);
 $UI->db->where("a.CallHistoryCallDate>='{$Out->get_value('start_date','StartDate')}'", "", false);
 $UI->db->where("a.CallHistoryCallDate<='{$Out->get_value('end_date','EndDate')}'", "", false);
...
//echo $UI->db->print_out();
 ...
 $rs = $UI->db->get();
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignLeader']]['tot_data_atempt'] +=($rows['status_601']+$rows['status_602']+$rows['status_603']);
...
// ------------------ solicited ----------------------------------------------------	
 ...
 $tot_data_atempt = $tot_connected_call;// ( $call_data_content['tot_data_atempt'] ? $call_data_content['tot_data_atempt'] : 0 );
...
// ===================== NEW DIS DATA ==============================================
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$arr_dis_new_data[$rows['AssignSelerId']] = explode(',',$rows['tot_new_data']);
...
// ==================== SOLICITED NEW / TMR  ===================================
  ...
  $UI->db->select("
			...
			a.SPVID as AsignLeader,
			a.AgentId as AssignSelerId,
			count(a.CustomerId) as tot_utilize_new,
			GROUP_CONCAT( distinct a.CustomerId) as arr_utilize_new_id ", FALSE);
  $UI->db->from("t_gn_callhistory_newutil a ");
  $UI->db->where_in("a.CampaignId", $CampaignId);
  ...
  $UI->db->where_in("a.SPVID", array_map('intval',  array($spvId)));
  $UI->db->where_in("a.AgentId", array_map('intval', $TmrId ));
  $UI->db->where("a.SessionReport", $gReportSession);
  $UI->db->group_by(array('AsignLeader','AssignSelerId'));
 // $UI->db->print_out();
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['AsignLeader']][$rows['AssignSelerId']] = explode(',', $rows['arr_utilize_new_id']);
	$data[$rows['AssignSelerId']]['data_new_utilize'] += $rows['tot_utilize_new'];
	$data[$rows['AssignSelerId']]['data_all_solicited'] += $rows['tot_utilize_new'];
 }
...
// ==================== SOLICITED OLD / TMR  ===================================
    ...
    $UI->db->select("
		DATE(a.CallHistoryCallDate) AS tgl,
		count(distinct a.CustomerId) as tot_old_utilize,
		c.UserId as AsignLeader,	
		a.CreatedById as AgentCode, 
		a.CustomerId as CustomerId", FALSE);
	$UI->db->from("t_gn_callhistory a FORCE INDEX(PRIMARY)");
	$UI->db->join("t_gn_customer b","a.CustomerId=b.CustomerId","INNER JOIN");
	$UI->db->join("tms_agent c ","a.SPVCode = c.id", "LFET");
	...
	$UI->db->where_in("e.UserId", array_map('intval', $obj->get_array_value('ManagerId')));
 	$UI->db->group_by(array("AsignLeader","AgentCode","CustomerId"));
	//$UI->db->print_out();
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$arr_skip_data = $arr_utilize_new[$rows['AsignLeader']][$rows['AgentCode']];
		if( !in_array($rows['CustomerId'], $arr_skip_data)) {
			$data[$rows['AgentCode']]['data_old_utilize'] += $rows['tot_old_utilize'];
			$data[$rows['AgentCode']]['data_all_solicited'] += $rows['tot_old_utilize'];
		}
	}
	
// ================ GET DATA ATEMPT PER ATM SUMMARY 11=======================================
...
FALSE);
	...
	foreach( $rs->result_assoc() as $rows ) 
 {
	...
	$data[$rows['AssignSelerId']]['tot_data_status_000']+=$rows['status_000'];
	$data[$rows['AssignSelerId']]['tot_data_status_101']+=$rows['status_101'];
	$data[$rows['AssignSelerId']]['tot_data_status_102']+=$rows['status_102'];
	$data[$rows['AssignSelerId']]['tot_data_status_103']+=$rows['status_103'];
	$data[$rows['AssignSelerId']]['tot_data_status_201']+=$rows['status_201'];
	$data[$rows['AssignSelerId']]['tot_data_status_202']+=$rows['status_202'];
	$data[$rows['AssignSelerId']]['tot_data_status_203']+=$rows['status_203'];
	$data[$rows['AssignSelerId']]['tot_data_status_204']+=$rows['status_204'];
	$data[$rows['AssignSelerId']]['tot_data_status_205']+=$rows['status_205'];
	$data[$rows['AssignSelerId']]['tot_data_status_206']+=$rows['status_206'];
	$data[$rows['AssignSelerId']]['tot_data_status_207']+=$rows['status_207'];
	$data[$rows['AssignSelerId']]['tot_data_status_208']+=$rows['status_208'];
	$data[$rows['AssignSelerId']]['tot_data_status_209']+=$rows['status_209'];
	$data[$rows['AssignSelerId']]['tot_data_status_210']+=$rows['status_210'];
	$data[$rows['AssignSelerId']]['tot_data_status_211']+=$rows['status_211'];
	$data[$rows['AssignSelerId']]['tot_data_status_301']+=$rows['status_301'];
	$data[$rows['AssignSelerId']]['tot_data_status_302']+=$rows['status_302'];
	$data[$rows['AssignSelerId']]['tot_data_status_303']+=$rows['status_303'];
	$data[$rows['AssignSelerId']]['tot_data_status_304']+=$rows['status_304'];
	$data[$rows['AssignSelerId']]['tot_data_status_305']+=$rows['status_305'];
	$data[$rows['AssignSelerId']]['tot_data_status_306']+=$rows['status_306'];
	$data[$rows['AssignSelerId']]['tot_data_status_307']+=$rows['status_307'];
	$data[$rows['AssignSelerId']]['tot_data_status_308']+=$rows['status_308'];
	$data[$rows['AssignSelerId']]['tot_data_status_309']+=$rows['status_309'];
	$data[$rows['AssignSelerId']]['tot_data_status_310']+=$rows['status_310'];
	$data[$rows['AssignSelerId']]['tot_data_status_311']+=$rows['status_311'];
	$data[$rows['AssignSelerId']]['tot_data_status_312']+=$rows['status_312'];
	$data[$rows['AssignSelerId']]['tot_data_status_313']+=$rows['status_313'];
	$data[$rows['AssignSelerId']]['tot_data_status_314']+=$rows['status_314'];
	$data[$rows['AssignSelerId']]['tot_data_status_315']+=$rows['status_315'];
	$data[$rows['AssignSelerId']]['tot_data_status_316']+=$rows['status_316'];
	$data[$rows['AssignSelerId']]['tot_data_status_317']+=$rows['status_317'];
	$data[$rows['AssignSelerId']]['tot_data_status_701']+=$rows['status_701'];
	$data[$rows['AssignSelerId']]['tot_data_status_401']+=$rows['status_401'];
	$data[$rows['AssignSelerId']]['tot_data_status_402']+=$rows['status_402'];
	$data[$rows['AssignSelerId']]['tot_data_status_319']+=$rows['status_319'];
 }
  
  
  // ================ GET DATA ATEMPT PER ATM SUMMARY 12=======================================
 ...
 $UI->db->select("
		...
		SUM(DISTINCT IF(f.CallReasonCode IN ('601'),1,0)) as status_601,
		SUM(DISTINCT IF(f.CallReasonCode IN ('602'),1,0)) as status_602,
		SUM(DISTINCT IF(f.CallReasonCode IN ('603'),1,0)) as status_603", 
FALSE);
 ...
 $rs = $UI->db->get();
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignSelerId']]['tot_data_atempt'] += ( $rows['status_601']+$rows['status_603']+$rows['status_603']);
	$data[$rows['AssignSelerId']]['tot_data_status_601']+=$rows['status_601'];
	$data[$rows['AssignSelerId']]['tot_data_status_602']+=$rows['status_602'];
	$data[$rows['AssignSelerId']]['tot_data_status_603']+=$rows['status_603'];
 }
...
// ------------------ solicited ----------------------------------------------------	
 ...
 $tot_data_atempt = $tot_connected_all; //( $call_data_content['tot_data_atempt'] ? $call_data_content['tot_data_atempt'] : 0 );
