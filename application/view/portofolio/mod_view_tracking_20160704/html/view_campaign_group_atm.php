...
// ==================== SOLICITED NEW / ATM  ===================================
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	//$arr_utilize_new[$rows['AssignSellerId']] = explode(',', $rows['arr_utilize_new_id']);
 ...
 // ================== SOLICITED OLD / ATM/SPV  ============================
	...
	$UI->db->where_in("e.UserId", array_map('intval', $Out->get_array_value('ManagerId')));
 	$UI->db->group_by(array("AssignSpv","AssignLeader", "AgentId","CustomerId"));
...
// ------------------ solicited ----------------------------------------------------	
 ...
 $tot_data_atempt = ($tot_connected_call? $tot_connected_call:0); //( $call_data_content['tot_data_atempt'] ? $call_data_content['tot_data_atempt'] : 0 );
...
// ===================== SIZE DIS NEW DATA ============================
 ...
 $UI->db->select("
	...
	a.AssignSelerId as AssignSelerId,
	a.AssignLeader as AssignLeader,
	COUNT( distinct a.CustomerId) as tot_new_data", FALSE);
 $UI->db->from("t_gn_assignment_log a force index(primary)");
 $UI->db->join("t_gn_customer b","a.CustomerId=b.CustomerId", "LEFT OUTER");
 $UI->db->where("a.CallReasonId=99", "", FALSE);
 $UI->db->where_in("b.CampaignId", $CampaignId); 
...
// ==================== SOLICITED NEW / LEADER  ===================================
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['AssignLeader']][$rows['AgentId']] = explode(',', $rows['arr_utilize_new_id']);
 ...
 // ================== SOLICITED OLD / LEADER  ============================
	...
	$UI->db->where_in("e.UserId", array_map('intval', $obj->get_array_value('ManagerId')));
 	$UI->db->group_by(array("AssignLeader", "AgentId","CustomerId"));
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$arr_skip_data = $arr_utilize_new[$rows['AssignLeader']][$rows['AgentId']];
		if( !in_array($rows['CustomerId'], $arr_skip_data)) {
