 ...
 */
 ...
 function _select_campaign_group_spv_summary()
{
...
// ------------- dis new DB  -------------------------
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data,a.AssignLeader as AssignLeader", FALSE);
...
// ------------- Old  DB  -------------------------
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_re_assigned,a.AssignLeader as AssignLeader", FALSE);
 $UI->db->from("t_gn_assignment_log a force index(primary)");
 $UI->db->join("t_gn_customer b","a.CustomerId=b.CustomerId", "LEFT OUTER");
 $UI->db->where("a.CallReasonId<>99", "", FALSE);
 $UI->db->where_in("b.CampaignId", $CampaignId); 
...
//----------------- utilize data new ----------
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignLeader']]['data_new_utilize'] += $rows['data_new_utilize'];
 } 
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignLeader']]['data_old_utilize'] += $rows['data_old_utilize'];
 } 
...
// --- load status in history Hight db -----------------------------
 ...
 $UI->db->reset_select();
 $UI->db->select(" 
	DISTINCT(a.CustomerId), a.CallReasonId , c.UserId as AssignLeader, a.CustomerId, ( 
	SELECT 
		cs.CallReasonCode 
	FROM t_gn_callhistory ts  
	INNER join t_lk_callreason cs on cs.CallReasonId=ts.CallReasonId 
	WHERE ts.CustomerId = a.CustomerId  and `ts`.`HistoryType` =  0 
	AND ts.SPVCode = c.id
	AND ts.CallHistoryCallDate>='{$Out->get_value('start_date','StartDate')}'
	AND ts.CallHistoryCallDate<='{$Out->get_value('end_date','EndDate')}'
	ORDER by ts.CallHistoryId DESC LIMIT 1 ) as CallreasonIdCode, 1 as total_size ", FALSE);
...
//---- filter data ---------------------------
 ...
 $UI->db->where("a.CallHistoryCallDate>='{$Out->get_value('start_date','StartDate')}'", "", false);
 $UI->db->where("a.CallHistoryCallDate<='{$Out->get_value('end_date','EndDate')}'", "", false);
 $UI->db->group_by(array("c.UserId","a.CustomerId"));
 $UI->db->order_by("a.CallReasonId", "DESC");
...
//echo $UI->db->print_out();
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$data[$rows['AssignLeader']]['tot_data_status_101'] += ( in_array($rows['CallreasonIdCode'],array('101')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_102'] += ( in_array($rows['CallreasonIdCode'],array('102')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_103'] += ( in_array($rows['CallreasonIdCode'],array('103')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_201'] += ( in_array($rows['CallreasonIdCode'],array('201')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_202'] += ( in_array($rows['CallreasonIdCode'],array('202')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_203'] += ( in_array($rows['CallreasonIdCode'],array('203')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_204'] += ( in_array($rows['CallreasonIdCode'],array('204')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_205'] += ( in_array($rows['CallreasonIdCode'],array('205')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_206'] += ( in_array($rows['CallreasonIdCode'],array('206')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_207'] += ( in_array($rows['CallreasonIdCode'],array('207')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_208'] += ( in_array($rows['CallreasonIdCode'],array('208')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_209'] += ( in_array($rows['CallreasonIdCode'],array('209')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_210'] += ( in_array($rows['CallreasonIdCode'],array('210')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_211'] += ( in_array($rows['CallreasonIdCode'],array('211')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_301'] += ( in_array($rows['CallreasonIdCode'],array('301')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_302'] += ( in_array($rows['CallreasonIdCode'],array('302')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_303'] += ( in_array($rows['CallreasonIdCode'],array('303')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_304'] += ( in_array($rows['CallreasonIdCode'],array('304')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_305'] += ( in_array($rows['CallreasonIdCode'],array('305')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_306'] += ( in_array($rows['CallreasonIdCode'],array('306')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_307'] += ( in_array($rows['CallreasonIdCode'],array('307')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_308'] += ( in_array($rows['CallreasonIdCode'],array('308')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_309'] += ( in_array($rows['CallreasonIdCode'],array('309')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_310'] += ( in_array($rows['CallreasonIdCode'],array('310')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_311'] += ( in_array($rows['CallreasonIdCode'],array('311')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_312'] += ( in_array($rows['CallreasonIdCode'],array('312')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_313'] += ( in_array($rows['CallreasonIdCode'],array('313')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_314'] += ( in_array($rows['CallreasonIdCode'],array('314')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_315'] += ( in_array($rows['CallreasonIdCode'],array('315')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_316'] += ( in_array($rows['CallreasonIdCode'],array('316')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_317'] += ( in_array($rows['CallreasonIdCode'],array('317')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_318'] += ( in_array($rows['CallreasonIdCode'],array('318')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_401'] += ( in_array($rows['CallreasonIdCode'],array('401')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_402'] += ( in_array($rows['CallreasonIdCode'],array('402')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_601'] += ( in_array($rows['CallreasonIdCode'],array('601')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_602'] += ( in_array($rows['CallreasonIdCode'],array('602')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignLeader']]['tot_data_status_603'] += ( in_array($rows['CallreasonIdCode'],array('603')) ? $rows['total_size'] : 0 );
 ...
 // ---------- get data talktime --------------
  ...
  $UI->db->where_in("c.AssignLeader", array_map('intval', $Out->get_array_value('spvId')));
...
// --------------- loop --------------------------------
 ...
 if(is_array($SpvId ) ) 
	 foreach( $SpvId  as $key => $AssignSelerId )
{
 ...
 */
 ...
 function _select_campaign_group_per_spv_detail( $spvId =0 )
{
	$UI=& get_instance(); // load db ;
	
/* ------------------------------------------------------------- */
	$cls=& get_class_instance("M_DataTrackingReport");
	$obj=& new EUI_Object( _get_all_request() );
    $TmrId = array_keys($cls->_select_report_tmr_by_spv($spvId));
...
echo "<table class=\"data\" border=1 style=\"border-collapse: collapse\">
	 <tr>
	  <td class=\"head\" rowspan=3 align=\"center\" style=\"vertical-align:middle;\">TMR</td>
	  <td class=\"head\" colspan=6>DB</td>
	  <td class=\"head\" rowspan=3>Attempted</td>
	  <td class=\"head\" rowspan=3>Attempt Ratio</td>
	  <td class=\"head\" colspan=5>Connected</td>
...
// ------------- dis new DB  -------------------------
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data,a.AssignSelerId as AssignSelerId", FALSE);
 ...
 $UI->db->where_in("a.AssignSpv", array_map('intval',  $obj->get_array_value('AtmId')));
 $UI->db->where_in("a.AssignLeader",array($spvId));
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$data[$rows['AssignSelerId']]['tot_new_assigned'] += $rows['tot_new_data'];
} 
...
// ------------- Old  DB  -------------------------
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_re_assigned,a.AssignSelerId as AssignSelerId", FALSE);
 $UI->db->from("t_gn_assignment_log a ");
 $UI->db->join("t_gn_customer b","a.CustomerId=b.CustomerId", "LEFT OUTER");
 $UI->db->where("a.CallReasonId<>99", "", FALSE);
 $UI->db->where_in("b.CampaignId", $CampaignId); 
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$data[$rows['AssignSelerId']]['tot_re_assigned'] += $rows['tot_re_assigned'];
} 
...
//----------------- utilize data new ----------
 ...
 $UI->db->select("
	count(distinct a.CustomerId) as data_new_utilize,
	a.CreatedById as AssignSelerId",FALSE);
 ...
 $UI->db->where_in("c.UserId", array_map('intval',  array($spvId)));
 $UI->db->where_in("CreatedById", array_map('intval', $TmrId ));
 
 $UI->db->group_by(array("a.CreatedById"));
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignSelerId']]['data_new_utilize'] += $rows['data_new_utilize'];
 } 
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignSelerId']]['data_old_utilize'] += $rows['data_old_utilize'];
 } 
...
// ---------- ambil data atemp pertiap Agent Under & spv & atm & MGR --------------
 ...
 $UI->db->reset_select();
 $UI->db->select("count(a.CallHistoryId) as data_size_atempt,
	a.CreatedById as AssignSelerId,
	(select ts.UserId from tms_agent ts where ts.id = a.SPVCode) as AssignLeader,
	(select ts.UserId from tms_agent ts where ts.id = a.ATMCode) as AssignSpv,
	(select ts.UserId from tms_agent ts where ts.id = a.AMGRCode) as AssignAmgr", 
FALSE);
 ...
 $rs = $UI->db->get();
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignSelerId']]['tot_data_atempt'] +=$rows['data_size_atempt'];
 }
...
// --- load status in history Hight db -----------------------------
 ...
 $UI->db->reset_select();
 $UI->db->select(" 
	DISTINCT(a.CustomerId), a.CallReasonId , a.CreatedById as AssignSelerId, a.CustomerId, ( 
	SELECT 
		cs.CallReasonCode 
	FROM t_gn_callhistory ts  
	INNER join t_lk_callreason cs on cs.CallReasonId=ts.CallReasonId 
	WHERE ts.CustomerId = a.CustomerId  
	AND ts.HistoryType=  0 
	AND ts.CreatedById = a.CreatedById
	AND ts.SPVCode = c.id
	AND ts.CallHistoryCallDate>='{$obj->get_value('start_date','StartDate')}'
	AND ts.CallHistoryCallDate<='{$obj->get_value('end_date','EndDate')}'
	ORDER by ts.CallHistoryId DESC LIMIT 1 ) as CallreasonIdCode, 1 as total_size ", FALSE);
 ...
 $UI->db->where_in("c.UserId", array_map('intval', array($spvId) ));
 $UI->db->where_in("a.CreatedById", array_map("intval", $TmrId) );
 $UI->db->group_by(array("a.CreatedById","a.CustomerId"));
 $UI->db->order_by("a.CallReasonId", "DESC");
...
//echo $UI->db->print_out();
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$data[$rows['AssignSelerId']]['tot_data_status_101'] += ( in_array($rows['CallreasonIdCode'],array('101')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_102'] += ( in_array($rows['CallreasonIdCode'],array('102')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_103'] += ( in_array($rows['CallreasonIdCode'],array('103')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_201'] += ( in_array($rows['CallreasonIdCode'],array('201')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_202'] += ( in_array($rows['CallreasonIdCode'],array('202')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_203'] += ( in_array($rows['CallreasonIdCode'],array('203')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_204'] += ( in_array($rows['CallreasonIdCode'],array('204')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_205'] += ( in_array($rows['CallreasonIdCode'],array('205')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_206'] += ( in_array($rows['CallreasonIdCode'],array('206')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_207'] += ( in_array($rows['CallreasonIdCode'],array('207')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_208'] += ( in_array($rows['CallreasonIdCode'],array('208')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_209'] += ( in_array($rows['CallreasonIdCode'],array('209')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_210'] += ( in_array($rows['CallreasonIdCode'],array('210')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_211'] += ( in_array($rows['CallreasonIdCode'],array('211')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_301'] += ( in_array($rows['CallreasonIdCode'],array('301')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_302'] += ( in_array($rows['CallreasonIdCode'],array('302')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_303'] += ( in_array($rows['CallreasonIdCode'],array('303')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_304'] += ( in_array($rows['CallreasonIdCode'],array('304')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_305'] += ( in_array($rows['CallreasonIdCode'],array('305')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_306'] += ( in_array($rows['CallreasonIdCode'],array('306')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_307'] += ( in_array($rows['CallreasonIdCode'],array('307')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_308'] += ( in_array($rows['CallreasonIdCode'],array('308')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_309'] += ( in_array($rows['CallreasonIdCode'],array('309')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_310'] += ( in_array($rows['CallreasonIdCode'],array('310')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_311'] += ( in_array($rows['CallreasonIdCode'],array('311')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_312'] += ( in_array($rows['CallreasonIdCode'],array('312')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_313'] += ( in_array($rows['CallreasonIdCode'],array('313')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_314'] += ( in_array($rows['CallreasonIdCode'],array('314')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_315'] += ( in_array($rows['CallreasonIdCode'],array('315')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_316'] += ( in_array($rows['CallreasonIdCode'],array('316')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_317'] += ( in_array($rows['CallreasonIdCode'],array('317')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_318'] += ( in_array($rows['CallreasonIdCode'],array('318')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_401'] += ( in_array($rows['CallreasonIdCode'],array('401')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_402'] += ( in_array($rows['CallreasonIdCode'],array('402')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_601'] += ( in_array($rows['CallreasonIdCode'],array('601')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_602'] += ( in_array($rows['CallreasonIdCode'],array('602')) ? $rows['total_size'] : 0 );
	$data[$rows['AssignSelerId']]['tot_data_status_603'] += ( in_array($rows['CallreasonIdCode'],array('603')) ? $rows['total_size'] : 0 );
 ...
 // ---------- get data talktime --------------
  ...
  $UI->db->reset_select();
  $UI->db->select("c.AssignSelerId as AssignSelerId, SUM(a.duration) as tot_talk_time ", FALSE);
  $UI->db->from("cc_recording a");
  $UI->db->join("t_gn_customer b "," a.assignment_data=b.CustomerId", "LEFT");
  $UI->db->join("t_gn_assignment c "," b.CustomerId=c.CustomerId","LEFT");
  ...
  $UI->db->where_in("c.AssignSpv", array_map('intval', $obj->get_array_value('AtmId')));
  $UI->db->where("c.AssignLeader",$spvId);
  $UI->db->where_in("c.AssignSelerId", array_map('intval', $TmrId) );
  $UI->db->group_by(array("AssignSelerId"));
 ...
 // $UI->db->print_out();
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$data[$rows['AssignSelerId']]['data_tot_talk_time'] +=$rows['tot_talk_time'];
  }
...
// ---------------- get data policy achivement per product ----------------------
 ...
 $UI->db->select("
	b.ProductId as ProductId,
	e.AssignSelerId as AssignSelerId,
	COUNT(a.PolicyId) as tot_nos_per_product,
	COUNT( distinct a.PolicyNumber) as tot_policy_per_product,
	SUM(a.PolicyPremi) as tot_premi_per_product,
	SUM(IF(c.PayModeId IN(2), (a.PolicyPremi * 12), a.PolicyPremi)) as tot_premi_anp_per_product", FALSE);
 ...
 $UI->db->where_in("e.AssignSpv", array_map("intval",  $obj->get_array_value('AtmId')));
 $UI->db->where_in("e.AssignLeader", array_map("intval", $obj->get_array_value('spvId')));
 $UI->db->where_in("e.AssignSelerId", array_map("intval", $TmrId));
 ...
 $UI->db->group_by(array("AssignSelerId","ProductId"));
 ...
 if($rs->num_rows() > 0)  
	foreach( $rs->result_assoc() as $rows )
 {
	$data[$rows['AssignSelerId']][$rows['ProductId']]['tot_nos_per_product']+= $rows['tot_nos_per_product'];
	$data[$rows['AssignSelerId']][$rows['ProductId']]['tot_policy_per_product']+= $rows['tot_policy_per_product'];
	$data[$rows['AssignSelerId']][$rows['ProductId']]['tot_premi_per_product']+= $rows['tot_premi_per_product'];
	$data[$rows['AssignSelerId']][$rows['ProductId']]['tot_premi_anp_per_product']+= $rows['tot_premi_anp_per_product'];
 }
...
// ------------ data Coloseint policy sales ----------------------------------------------
  ...
  $UI->db->where("a.PolicySalesDate>='{$obj->get_value('start_date','StartDate')}'", "", false);
  $UI->db->where("a.PolicySalesDate<='{$obj->get_value('end_date','EndDate')}'", "", false);
  $UI->db->group_by(array("PolicyNumber","AssignSelerId"));
 // $UI->db->print_out();
  ...
  if($rs->num_rows() > 0)  
		foreach( $rs-> result_assoc() as $rows )
{
	$data[$rows['AssignSelerId']]['total_data_policy'] += $rows['tot'];
	$data[$rows['AssignSelerId']]['tot_data_insured'] += $rows['tot_data_insured'];
	$data[$rows['AssignSelerId']]['tot_data_premi'] += $rows['tot_data_premi'];
	$data[$rows['AssignSelerId']]['tot_data_premi_anp'] += $rows['tot_data_premi_anp'];
	
}	

// ------------ select count user login on interval ---------------------------------
 $UI->db->reset_select();
 $UI->db->select("count(distinct a.ActivityUserId) as tot_user_tmr, a.ActivityUserId", FALSE);
 $UI->db->from("t_gn_activitylog a force index(primary) ");
 $UI->db->join("tms_agent b ","a.ActivityUserId=b.UserId", "LEFT");
 ...
 if($rs->num_rows() > 0)  
	foreach( $rs-> result_assoc() as $rows )
{
	$data[$rows['ActivityUserId']]['data_tot_user_login'] += $rows['tot_user_tmr'];
}
...
// ---------- get total work of hour in list ---
 ...
 $UI->db->reset_select();
 $UI->db->select(" a.ActivityUserId as UserId, date(a.ActivityDate) as tgl",FALSE);
 $UI->db->from("t_gn_activitylog a force index(primary) ");	
 $UI->db->join("tms_agent b","a.ActivityUserId = b.UserId", "LEFT");
 $UI->db->where("a.ActivityDate>='{$obj->get_value('start_date','StartDate')}'", "", FALSE);
 $UI->db->where("a.ActivityDate<='{$obj->get_value('end_date','EndDate')}'", "", FALSE);
	...
	foreach( $rs-> result_assoc() as $rows )
 {
		...
		AND $row = $qry->result_first_assoc() )
	{
		if( count($row) > 0 ) { 
			$hour_maximum = ( (int)$row['logout'] - (int)$row['login']);
			$data[$rows['UserId']]['data_tot_hour_work'] += $hour_maximum;
		}
	}	
 }
...
// --------------- loop --------------------------------
 ...
 if(is_array($TmrId) ) 
	 foreach( $TmrId as $key => $AssignSelerId )
{
 ...
 */
 ...
 function _select_campaign_group_spv_detail()
{
 $vars = new EUI_Object(_get_all_request() );
 $class =& get_class_instance('M_SysUser');
 
 if( count($vars) ) 
	foreach( $vars->get_array_value('spvId') as $k => $gSpvId )
 {
	$Instance = new EUI_Object( $class->_getUserDetail( $gSpvId ));
	echo "<h4>{$Instance->get_value('Username')}</h4><hr size=1>"; 
	_select_campaign_group_per_spv_detail( $gSpvId );
 ...
 function showReport()
{
	if( _get_post('interval') =='summary' ){
		_select_campaign_group_spv_summary();		
	}
	
	
	if( _get_post('interval') =='detail' ){
		_select_campaign_group_spv_detail();		
	}
} 	
 ...
 function showheaders()
{
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Data Tracking - Campaign Group By SPV</p>".
