...
if( is_array($call_users) )  
	foreach( $call_users as $tgl_index => $tgl_name )
{
...
// --------- index --------------------------------------------------------------------- 
 $tgl_name = _getDateIndonesia($tgl_name);
 $call_data_content = $call_content[$tgl_index];
...
echo "<tr height=28 bgcolor={$bg_color}>
	  <td class='{$css_val} border-top-none-left-yes font-align-left-color-696868' nowrap>{$tgl_name}</td>
