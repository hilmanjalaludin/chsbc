...
<script type="text/javascript">
	...
	Ext.DOM.exClearCmpPds = function(status){
		
		Ext.Ajax ({
			url 	: Ext.EventUrl(["SetCampaign","ClearCmpPds"]).Apply(),
			method 	: 'POST',
			param 	: 
			{
				CampaignId : CampaignId_PDS
			...
			{
				Ext.Util(e).proc(function( response ){
					console.log( "## ClearCmpPds" ); console.log( response);
					if(response.status){
						Ext.Msg("Clear Campaign PDS Success").Info();
						Ext.DOM.Search();
					}else{
						Ext.Msg("Clear Campaign Fail/Tidak ada data").Info();
						Ext.DOM.Search();
					}
				});	
			}
	...
	Ext.DOM.ClearAgentPds = function(){
		Ext.Ajax ({
			url 	: Ext.EventUrl(["SetCampaign","ClearAgentPds"]).Apply(),
			method 	: 'POST',
			param 	: 
			{
				CampaignId : CampaignId_PDS
			...
			{
				Ext.Util(e).proc(function( response ){
					console.log( "## ClearAgentPds" ); console.log( response);
					if(response.status){
						Ext.Msg("Clear Agent PDS Success").Info();
						Ext.DOM.Search();
					}else{
						Ext.Msg("Clear Agent PDS Fail/Tidak ada data").Info();
						Ext.DOM.Search();
					}
				});	
			}
	...
	Ext.DOM.ClearCmpPds = function(e)
	{
		var CampaignId =Ext.Cmp('CampaignId').getValue();
		  if( CampaignId == '' ){
			Ext.Msg("Please select a rows ").Info();
		...
		Ext.Ajax ({
			...
			method 	: 'POST',
			param 	: {CampaignId : CampaignId_PDS },
			ERROR : function( e )
			{
				Ext.Util(e).proc(function( response ){
					console.log( response);
					var pdsstatusmsg = {0:'Reset data PDS!?', 1:'PDS is Running!, PDS Campaign Must be Stoped!', 2:'PDS is Paused!, PDS Campaign Must be Stoped!'};
					if(response.status){
						alert(pdsstatusmsg[response.status]);
						return false;
					}else{
						var clearconfirm = confirm(pdsstatusmsg[response.status]);
						if(clearconfirm){
							Ext.DOM.exClearCmpPds( response.status );
						}else{
							return false;
						}
					}
	...
	$(document).ready(function()
 	{
		$('#toolbars').extToolbars({
			...
			extTitle :[['Search'],['Clear'],['Edit'],['Add Campaign PDS'],['Clear Cmp PDS'],['Clear Agent PDS'],['Start PDS'],['Pause PDS'],['Stop PDS'],['Resum PDS']],
			extMenu  :[['Search'],['Clear'],['EditPds'],['Add'],['ClearCmpPds'],['ClearAgentPds'],['StartPds'],['PausePds'],['StopPds'],['ResumPds']],
			extIcon  :[['zoom.png'],['cancel.png'],['application_form_edit.png'],['add.png'],['cancel.png'],['cancel.png'],['telephone_go.png'],['control_pause_blue.png'],['control_stop_blue.png'],['telephone_go.png']],
			extText  :true,
			extInput :true,
			extOption:[]
		});
