...
Ext.DOM.Submit = function()
{
	...
	var result= false;
	var field = ['CampaignNumber', 'CampaignName','CampaignCode','ListGroupCampaign','CampaignCallLimit','StartDate','CampaignCallFactor',
				 'CampaignAbandonRate','CampaignCallWait','CampaignCallRetry','ListCampaignCallPrefarance','StatusActive','CampaignCallRetryAfter'];
	var chosen;
	...
	var numericFieldResult = false;
	var numericfield = ['CampaignCallLimit', 'CampaignCallFactor', 'CampaignAbandonRate', 'CampaignCallWait', 'CampaignCallRetry', 'StatusActive', 'CampaignCallRetryAfter'];
	$.each( numericfield, function( key, valuel ) {
		var z = Ext.Cmp(valuel).getValue();
		if(!z.match(/^\d+/)){
			$("#"+valuel).css('border','1px solid red');
			chosen ="#"+valuel+"_chosen"; 
			$(chosen).css('border','1px solid red');
			numericFieldResult = true;
		}
	});
	
	if(numericFieldResult){
		Ext.Msg("Red Outlined Field Should be Numeric Only!").Info();
		numericfield = null;
		return false;
	}
...
Ext.DOM.UpdatePds = function()
{
	...
	var result = false;
	var field  = ['CampaignNumber', 'CampaignName','CampaignCode','ListGroupCampaign','CampaignCallLimit','CampaignCallFactor','CampaignAbandonRate','CampaignCallWait','CampaignCallRetry','ListCampaignCallPrefarance','StatusActive'];
	var chosen;
...
/* new line*/
 Ext.DOM.SelectSpvBySess = function( obj ){
	$('#CampaignSPVList').toogle({
		url : 'MgtAssignment/SelectUserBySess',
		param :  {
			timex : Ext.Date().getDuration(),
			handling : 3,
			obj : obj
		}
	});
 ...
 Ext.DOM.SelectTmrBySpv = function( obj ){
	 
	// var SPVlist = Ext.Cmp('CampaignSPVList').getValue();
	// console.log(SPVlist);
	$('#CampaignTMRListBySPV').toogle({
		url : 'MgtAssignment/SelectUserBySpvs',
		param :  {
			timex : Ext.Date().getDuration(),
			handling : 4
		},
		elval:['CampaignSPVList']
	});
 }
/* end of new line */
...
$(document).ready( function()
{
	...
	// var SPVlist = <?php echo ($row?$row->get_value('SPVGroup'):"NULL"); ?>;
	// var CallGrp = <?php echo ($row?$row->get_value('SPVGroup'):"NULL"); ?>;
	
	// Ext.DOM.SelectSpvBySess(SPVlist);
});
