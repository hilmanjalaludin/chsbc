...
var navigation = {
	custnav	 : Ext.DOM.INDEX +'/SetCampaignLinked/index/',
	custlist : Ext.DOM.INDEX +'/SetCampaignLinked/content/',
}
...
Ext.DOM.Edit = function(){
	Ext.Ajax
	({
		url    : Ext.DOM.INDEX +"/SetCampaignLinked/Edit/",
		method : "GET",
		param  : {
...
Ext.DOM.Add = function(){
	Ext.Ajax
	({
		url    : Ext.DOM.INDEX +"/SetCampaignLinked/Add/",
		method : "GET",
		param  : {
			time : '<?php echo time(); ?>' 
		}
...
Ext.DOM.Delete = function()
{
	...
	if(Ext.Cmp('chk_skill').empty()!=true)
	{
		...
		{
			Ext.Ajax({
				url 	: Ext.DOM.INDEX+'/SetCampaignLinked/Delete/',
				method 	:'POST',
				param 	: { Id : Id },
				ERROR : function(e){
					var ERR = JSON.parse(e.target.responseText);
					if( ERR.success ) {
						Ext.Msg("Delete link DID").Success();
						Ext.EQuery.postContent();
					}
					else{
						Ext.Msg("Delete link DID").Failed();
						return false;
					}
				}
			}).post();
