...
<fieldset class="corner" style="background-color:white;margin:3px;">
 <legend class="icon-application">&nbsp;&nbsp;&nbsp;Add Link DID Number </legend>	
...
<?php	
	...
	foreach( $FieldName as $key => $label ) 
	{
		if(!in_array($label, $HTML['primary'])) 
		{
			...
			{
				echo "<tr>
						<td class=\"text_caption\">* " . ucfirst($HTML['combo'][$label]['label']) . " </td>
						<td>" . form() -> combo($label,'select long', $HTML['combo'][$label]['data']) . " </td>
					</tr> ";
			}	
		}	
	} 
