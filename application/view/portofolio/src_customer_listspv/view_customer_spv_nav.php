...
<script type="text/javascript">
	...
	Ext.DOM.datas = {
		...
		//get input name 191919
		vartiering: '<?php echo _get_exist_session('vartiering'); ?>',
		AproveName: '<?php echo _get_exist_session('AproveName'); ?>'
	...
	Ext.DOM.navigation = {
		custnav: Ext.DOM.INDEX + '/SrcCustomerListSpv/index/',
		custlist: Ext.DOM.INDEX + '/SrcCustomerListSpv/Content/',
	}
	...
	Ext.DOM.SetFollowUp = function(CustomerId) {
		var data = Ext.Ajax({
			url: Ext.EventUrl(['SrcCustomerListSpv', 'SetFollowup']).Apply(),
			method: 'POST',
			param: {
	...
	window.EventRecscource = function() {
		...
		var windowDataUri = Ext.EventUrl(new Array('SrcCustomerListSpv/FilterRecData')),
	...
	Ext.DOM.load_Recsource_pull_src = function(obj) {
		$("#src_customerspv_recsource").toogle({
			url: 'SrcCustomerListSpv/filtered_recsource',
			param: {
				time: Ext.Date().getDuration()
			},
			elval: ['src_campaign_name', 'src_customerspv_keyword']
		});
	};
	/*
	 * @ package 		detail go customer data
	...
	Ext.DOM.getAge = function(CustomerId) {
		return (Ext.Ajax({
			url: Ext.EventUrl(new Array('SrcCustomerListSpv', 'get_age')).Apply(),
			method: 'GET',
			param: {
				CustomerId: CustomerId
	...
	Ext.DOM.gotoCallCustomer = function(CustomerId) {
		if (CustomerId != '') {
			...
			Ext.ShowMenu(new Array('SrcCustomerListSpv', 'ContactDetail'),
				Ext.System.view_file_name(), {
					CustomerId: CustomerId,
					ControllerId: 'SrcCustomerListSpv'
				});
			// } else {
			...
			<div class="ui-widget-form-table-compact" style="margin:0 10px;">
				<div class="ui-widget-form-row baris-1">
						...
						<div class="ui-widget-form-table-compact">
							...
							<div class="ui-widget-form-row baris-1">
								...
								<div class="ui-widget-form-cell text_caption left" style="vertical-align:top;">
									<?php echo form()->combo(
										'src_call_reason',
										'select long cz-original',
										CallResultAll(),
										get_cokie_array('src_call_reason'),
										null,
										array('multiple' => 'multiple')
									); ?></div>
								<!-- /di v -->
							</div>
							<!-- hilman & andi 12-12-2019 -->
							<div class="ui-widget-form-row baris-1">
								<div class="ui-widget-form-cell text_caption"><?php echo lang(array('Var Tiering')); ?></div>
								<div class="ui-widget-form-cell text_caption center">:</div>
								<div class="ui-widget-form-cell left"><?php echo form()->input('vartiering', 'input_text long', _get_exist_session('vartiering')); ?></div>


								<div class="ui-widget-form-cell text_caption"><?php echo lang(array('Qa Status')); ?></div>
								<div class="ui-widget-form-cell text_caption center">:</div>
								<div class="ui-widget-form-cell left"><?php echo form()->input('AproveName', 'input_text long', _get_exist_session('AproveName')); ?></div>
							</div>
							<!-- hilman & andi 12-12-2019 -->
						</div>
					</div>
					<div class="ui-widget-form-cell" style="vertical-align:top;">
						...
						<div class="ui-widget-form-row baris-1">
							<div class="ui-widget-form-cell text_caption" style="vertical-align:top;"><? #php echo lang(array('Recsource'));
																										?></div>
							<div class="ui-widget-form-cell text_caption center" style="vertical-align:top;">:</div>
							<div class="ui-widget-form-cell text_caption left" style="vertical-align:top;"><? #php echo form() -> listcombo('src_customer_recsource','select long',  $recs, explode(',',_get_exist_session('src_customer_recsource')) );
																											?></div>

							<div class="ui-widget-form-cell text_caption" style="vertical-align:top;"><? #php echo lang(array('Call Result'));
																										?></div>
							<div class="ui-widget-form-cell text_caption center" style="vertical-align:top;">:</div>
							<div class="ui-widget-form-cell text_caption left" style="vertical-align:top;"><? #php echo form() -> listcombo('src_call_reason','select long', $callreason, explode(',',_get_exist_session('src_call_reason')) );
																											?></div>
						</div>
					</di v -->
					</div>
