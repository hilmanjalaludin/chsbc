...
foreach($LoopUser as $ID => $rows){
	...
	foreach($RowData1[$ID] as $Data => $rows1){
		...
		$worksheet->write_string($arr_rows, 12, $CB);
		$worksheet->write_string($arr_rows, 13, $SA);
		$worksheet->write_string($arr_rows, 14, $INC);
		$worksheet->write_string($arr_rows, 15, $GPU);
		$worksheet->write_string($arr_rows, 16, $CPGP);
