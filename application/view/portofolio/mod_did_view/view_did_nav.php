...
var navigation = {
	custnav	 : Ext.DOM.INDEX +'/SetDIDNumber/index/',
	custlist : Ext.DOM.INDEX +'/SetDIDNumber/content/',
}
...
Ext.DOM.Edit = function(){
	Ext.Ajax
	({
		url    : Ext.DOM.INDEX +"/SetDIDNumber/Edit/",
		method : "GET",
		param  : {
...
Ext.DOM.Add = function(){
	Ext.Ajax
	({
		url    : Ext.DOM.INDEX +"/SetDIDNumber/Add/",
		method : "GET",
		param  : {
			time : '<?php echo time(); ?>' 
		}
...
Ext.DOM.Delete = function()
{
	...
	if(Ext.Cmp('chk_skill').empty()!=true)
	{
		...
		{
			Ext.Ajax({
				url 	: Ext.DOM.INDEX+'/SetDIDNumber/Delete/',
				method 	:'POST',
				param 	: { Id : Id },
				ERROR : function(e){
					var ERR = JSON.parse(e.target.responseText);
					if( ERR.success ) {
						Ext.Msg("Delete DID").Success();
						Ext.EQuery.postContent();
					}
					else{
						Ext.Msg("Delete DID").Failed();
						return false;
					}
				}
			}).post();
...
Ext.DOM.Save = function() {
  Ext.Ajax 
  ({
		url 	: Ext.DOM.INDEX+ '/SetDIDNumber/Save/',
		method 	:'POST',
...
Ext.DOM.Update = function() {
	Ext.Ajax 
	({
		url 	: Ext.DOM.INDEX+ '/SetDIDNumber/Update/',
		method 	:'POST',
