...
<script>
	Ext.DOM.ListAtm=function()
	{
		Ext.Ajax
		({
			url 	: Ext.DOM.INDEX +'/QAQualityAssuranceReport/GroupFilterBy/',
			param 	: {
	...
	Ext.DOM.ListTL=function()
	{
		Ext.Ajax
		({
			url 	: Ext.DOM.INDEX +'/QAQualityAssuranceReport/GroupFilterTL/',
			param 	: {
				FilterId : Ext.Cmp('spvname').getValue()
			}
		}).load('list_tl');
	}
	Ext.DOM.ListTSR=function()
	{
		Ext.Ajax
		({
			url		: Ext.DOM.INDEX +'/QAQualityAssuranceReport/GroupFilterTSR/',
			param	: {
	...
	Ext.DOM.ShowReport=function()
	{
		...
		else if(Ext.Cmp('tsrname').getValue()=='')
		{
			alert('Please Choose TSR!');
		}
	...
	Ext.DOM.ShowExcel = function()
	{
		Ext.Window
		({
			url 	: Ext.DOM.INDEX +'/QAQualityAssuranceReport/ShowExcel/',
			param 	: { 
				filter_by : Ext.Cmp('FilterBy').getValue(),
				spv_id : Ext.Cmp('spvname').getValue(),
				tl_id : Ext.Cmp('tlname').getValue(),
...
<div>
	<table cellpadding='4' cellspacing=4>
		...
		<tr>
			<td class="text_caption bottom">Telesales</td>
			<td><span id="list_tsr"><?php __(form()->combo('tsrname','select long', array(), null )); ?></span></td>
		</tr>
