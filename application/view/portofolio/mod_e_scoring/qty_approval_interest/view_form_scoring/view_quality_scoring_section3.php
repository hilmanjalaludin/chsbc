...
<table name="section1" class="scoring">
	...
	<tbody>
		<tr rowspan="2">
			...
			<td class="center">
				<select required="required" triggerto="section3_1_val<?= $formfor; ?>" name="Section3_1" class="section3<?= $formfor; ?>">
					...
					<option <?= _selected("No",$SSection1->Section3_1); ?> value="No"  fatal="true" valuejs="0">No</option>
				</select>	
				<br><br>		

				<select name="Remarks_Section3" class="Remarks_Section3">
					<option value="">- choose one -</option>
					<option value="Customer Request - Verbally acknowledged by the agent">Customer Request - Verbally acknowledged by the agent</option>
					<option value="Uncovered a need via discovery">Uncovered a need via discovery</option>
					<option value="Utilized an Individual solution">Utilized an Individual solution</option>
				</select>
		...
		<tr>
			...
			<td class="center totalscore_section3<?= $formfor; ?> totalsection<?= $formfor; ?>">
				<?php echo isset($fs->TotalScoreSec3) ? $fs->TotalScoreSec3 : 0; ?>
			</td>
		</tr>
	</tbody>
</table>