...
Ext.DOM.navigation = {
	custnav	 : Ext.DOM.INDEX +'/SetPromiseScript/index/',
	custlist : Ext.DOM.INDEX +'/SetPromiseScript/Content/',
}
...
// ------------------- get data script --------------------
	...
	({
			url     : Ext.EventUrl(['SetPromiseScript','ShowProductScript']).Apply(), 
			name    : 'WinProduct',
...
Ext.DOM.ShowWindowScriptView = function()
{
	// alert('View');
	// return false
...
// ------------------- get data script --------------------
	...
	var WindowScript = new Ext.Window 
	({
			url     : Ext.EventUrl(['SetPromiseScript','ShowProductScriptView']).Apply(), 
			name    : 'WinProduct',
			height  : ($(window).innerHeight()),
			width   : ($(window).innerWidth() - ( $(window).innerWidth()/2 )),
			left    : ($(window).innerWidth()/2),
...
Ext.DOM.Disable = function()
{
	...
	{
		Ext.Ajax({
			url 	: Ext.DOM.INDEX+ '/SetPromiseScript/SetActive/',
			method 	:'POST',
			param 	: {
				Active : 0,
				ScriptId : ScriptId
...
Ext.DOM.Delete = function()
{
	...
	if(Ext.Cmp('ScriptId').empty()!=true)
	{
		...
		{
			Ext.Ajax({
				url 	: Ext.DOM.INDEX+ '/SetPromiseScript/Delete/',
				method 	:'POST',
				param 	: {
					ScriptId : ScriptId
				},
