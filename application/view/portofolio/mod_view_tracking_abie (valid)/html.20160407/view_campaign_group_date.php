...
//=============== DIST NEW DATA ON DB ========================================================= 
 ...
 $UI->db->reset_select();
 $UI->db->select("count( distinct a.CustomerId) as tot_new_data, date(a.AssignDate) as tgl, 
	GROUP_CONCAT(distinct a.CustomerId) as arr_new_utilize", FALSE);
...
//=============== DIST OLD/RE-UTIL DATA ON DB ========================================================= 
 ...
 // $week_days = _select_report_list_weekdays();
 // if(is_array($week_days) )
	 // foreach( $week_days as $key => $tgl )
// {
	
// }

if( is_array($arr_new_utilize) and count($arr_new_utilize) > 0 )
  foreach( $arr_new_utilize as $start_date => $CustomerId )
 {
	$UI->db->reset_select();
	...
	$UI->db->where("a.CallReasonId<>99", "", FALSE);
	$UI->db->where_in("b.CampaignId", $CampaignId); 
	$UI->db->where("a.AssignDate>='$start_date 00:00:00'", "", false);
	$UI->db->where("a.AssignDate<='$start_date 23:59:59'", "", false);
	...
	{
		$data[$start_date]['tot_re_assigned']+= $rows['tot_re_assigned'];
	} 
 }	
...
// ==================== SOLICITED NEW ===================================
  ...
  $UI->db->select("
			...
			count(a.CustomerId) as tot_utilize_new", FALSE);
...
// ==================== SOLICITED OLD ===================================
 ...
 if(is_array($week_days) )
	 foreach( $week_days as $key => $tgl )
{
	$total_unutilize = _inialize_not_new_utilize_per_date( $tgl);
	
	$data[$tgl]['data_old_utilize']+= $total_unutilize;
	$data[$tgl]['data_all_solicited']+= $total_unutilize;
 }
