...
// --- load status in history Hight db -----------------------------
 ...
 $UI->db->select(" 
	...
	(  select cgs.CallReasonCode
	   ...
	   and tgs.ATMCode=d.id ) as CallreasonIdCode", FALSE);
...
// echo $UI->db->print_out();
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$data[$rows['AssignLeader']]['data_all_solicited'] += $rows['total_size'];
	$data[$rows['AssignLeader']]['data_old_utilize'] += $rows['tot_old_utilize'];
	$data[$rows['AssignLeader']]['data_new_utilize'] += $rows['tot_new_utilize'];
	...
	$data[$rows['AssignLeader']]['tot_data_status_701'] += ( in_array($rows['CallreasonIdCode'],array('701')) ? $rows['total_size'] : 0 );
}
...
// --------------------------------------------------------------------------------------------------------
 ...
 $sum_avg_data_reject_upfront_rate		= ( $sum_tot_contacted ? ( $sum_tot_status_701/$sum_tot_contacted) : 0);
...
// --- load status in history Hight db -----------------------------
 ...
 $UI->db->where_in("c.UserId", array_map('intval', array($spvId) ));
 $UI->db->where_in("a.CreatedById", array_map("intval", $TmrId) );
 $UI->db->group_by(array("AssignSelerId","a.CustomerId"));
...
//echo $UI->db->print_out();
 ...
 if( $rs->num_rows() > 0 ) 
	 foreach( $rs->result_assoc() as $rows ) 
{
	$data[$rows['AssignSelerId']]['data_all_solicited']+= $rows['total_size'];
	$data[$rows['AssignSelerId']]['data_old_utilize']+= $rows['tot_old_utilize'];
	$data[$rows['AssignSelerId']]['data_new_utilize']+= $rows['tot_new_utilize'];
	...
	$data[$rows['AssignSelerId']]['tot_data_status_701'] += ( in_array($rows['CallreasonIdCode'],array('701')) ? $rows['total_size'] : 0 );
...
$tot_connected_all =($tot_data_connected_y+$tot_data_connected_n101+$tot_data_connected_n102+$tot_data_connected_n103);						   
...
// --------------------------------------------------------------------------------------------------------
 ...
 $sum_tot_productivity_per_tmr_premi 	= _getCurrency($sum_tot_productivity_per_tmr_premi); 
 $sum_tot_avg_premi 					= _getCurrency($sum_tot_avg_premi);
 $sum_avg_data_reject_upfront_rate 		= _setPercent( ( $sum_tot_contacted ? ( $sum_tot_status_701 /$sum_tot_contacted ) : 0 ));
 ...
 function showheaders()
{
	echo "<div class=\"center\">".
		 "<p class=\"normal font-size22\">Report Call Tracking - Campaign Group By SPV</p>".
