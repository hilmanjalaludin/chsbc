...
// ==================== SOLICITED NEW / TMR  ===================================
 ...
 // $UI->db->print_out();
  ...
  if( $rs->num_rows() > 0 ) 
	foreach( $rs->result_assoc() as $rows ) 
 {
	$arr_utilize_new[$rows['tgl']][$rows['AssignSelerId']] = explode(',', $rows['arr_utilize_new_id']);
...
// ==================== SOLICITED OLD / TMR  ===================================
	...
	$UI->db->where_in("e.UserId", array_map('intval', $obj->get_array_value('ManagerId')));
 	$UI->db->group_by(array("tgl","AgentCode","CustomerId"));
	//$UI->db->print_out();
	$rs = $UI->db->get();
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$arr_skip_data = $arr_utilize_new[$rows['tgl']][$rows['AgentCode']];
		if( !in_array($rows['CustomerId'], $arr_skip_data)) {
