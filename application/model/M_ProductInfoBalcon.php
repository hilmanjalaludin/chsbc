<?php
class M_ProductInfoBalcon extends EUI_Model
{
...
//$sql = "select * from t_gn_loan_tiering a where a.CustomerId = '".$custid."' AND a.LoanAmount>0";
    ...
    $sql = "SELECT a.*,pup.*,
    (SELECT frm.DisburseAmount FROM t_gn_frm_pil_topup frm
    WHERE frm.CustomerId=a.CustomerId AND frm.Tenor=a.Tenor) AS DisburseAmount_1,
    (SELECT frm.Loan FROM t_gn_frm_pil_topup frm
    WHERE frm.CustomerId=a.CustomerId AND frm.Tenor=a.Tenor) AS Loan,
    (SELECT frm.Installment FROM t_gn_frm_pil_topup frm
    WHERE frm.CustomerId=a.CustomerId AND frm.Tenor=a.Tenor) AS INSTALL,
    (SELECT frm.oriinstallment FROM t_gn_frm_pil_topup frm
    WHERE frm.CustomerId=a.CustomerId AND frm.Tenor=a.Tenor) AS oriinstallment
    FROM t_gn_loan_tiering a
    LEFT JOIN t_gn_attr_pil_topup pup ON pup.CustomerId=a.CustomerId
    where a.CustomerId = '".$custid."' AND a.LoanAmount>0";
    if($tenor!=NULL){
    ...
    foreach( $qry -> result_assoc() as $rows ) {
        if($tenor!=NULL){
// var_dump($rows['disbusfrm']);
// die;
            $_datap['LoanAmount'] = $rows['LoanAmount'] == null ? $rows['LoanAmount'] : $rows['LoanAmount'];
// $_datap['Installment'] = $rows['Installment'];
            
// edit hilman&andi start
            $_datap['DisburseAmount'] = $rows['DisburseAmount_1'] == null ? $rows['DisburseAmount'] : $rows['DisburseAmount_1'];
            $_datap['Installment'] = $rows['INSTALL'] == null ? $rows['Installment'] : $rows['INSTALL'];
...
// edit hilman&andi end
            ...
            $_datap['oriinstallment'] = $rows['oriinstallment'];
            
            $_datap['NeedNPWP'] = $rows['NeedNPWP'];
            $_datap['OutstandingTenor'] = $rows['OutstandingTenor'];
            $_datap['Outstanding_Balance'] = $rows['Outstanding_Balance'];
            $_datap['Interest_rate_06'] = $rows['Interest_rate_06'];
            $_datap['PilProPremi'] = $rows['PilProPremi'];
            $_datap['TotalTenorNew'] = $rows['TotalTenorNew'];
            $_datap['Param']= $rows['DisburseAmount_1'] == null ? 'customer' : 'QA';
        }else{
// echo "<pre>";
// var_dump('loan', $rows['Loan']);
// var_dump('tenor', $rows['Tenor']);
// var_dump('LoanAmount', $rows['LoanAmount']);
// var_dump('asd');
            $_datap[$rows['Tenor']]['LoanAmount'] = $rows['LoanAmount'] == null ? $rows['LoanAmount'] : $rows['LoanAmount'];
// $_datap[$rows['Tenor']]['Installment'] = $rows['Installment'];
// echo "<pre>";
// var_dump($rows['Loan']);
            
// edit hilman&andi start
            $_datap[$rows['Tenor']]['DisburseAmount'] = $rows['DisburseAmount'] == null ? $rows['DisburseAmount'] : $rows['DisburseAmount'];
            
            $_datap[$rows['Tenor']]['Installment'] = $rows['Installment'] == null ? $rows['Installment'] : $rows['Installment'];
            
            $_datap[$rows['Tenor']]['AdminFee'] = $rows['AdminFee'];
            $_datap[$rows['Tenor']]['oriinstallment'] = $rows['oriinstallment'];
...
// edit hilman&andi end
            ...
            $_datap[$rows['Tenor']]['Outstanding_Balance'] = $rows['Outstanding_Balance'];
            $_datap[$rows['Tenor']]['Interest_rate_06'] = $rows['Interest_rate_06'];
            $_datap[$rows['Tenor']]['PilProPremi'] = $rows['PilProPremi'];
            $_datap[$rows['Tenor']]['TotalTenorNew'] = $rows['TotalTenorNew'];
            $_datap[$rows['Tenor']]['Param']= $rows['DisburseAmount_1'] == null ? 'QA' : 'customer';
        }
    }
    return $_datap;
}
...
// tutup edit hilman&andi start
    ...
    $this->db->reset_write();
// edit hilman&andi start
    $this->db->set('Installment' ,str_replace(',','',$out->get_value('Installment')));
    $this->db->set('AdminFee' ,$AdminFee);
    
    $this->db->set('DisburseAmount' ,str_replace(',','',$out->get_value('DisburseAmount')));
    $this->db->set('Loan' ,str_replace(',','',$out->get_value('DisburseAmount')));
    
    $this->db->set('tnc',$out->get_value('tnc'));
    $this->db->set('vartiering',$out->get_value('vartiering'));
    $this->db->set('oridisbrusmount',$loans['DisburseAmount']);
    $this->db->set('oriinstallment',$loans['Installment']);
    $this->db->set('oriloan',$loans['LoanAmount']);
// edit hilman&andi end
