...
class M_ProductInfoFlexi extends EUI_Model
{
	...
	function _get_data_loans($custid, $tenor = NULL){
		...
		$sql = "select * from t_gn_loan_tiering a where a.CustomerId = '".$custid."' AND a .Desc_Card is null ";
		//tutup edit irul
		//echo $sql;
  ...
  //        die();  
        if($out->get_value('vartiering_2nd')==100 || $out->get_value('vartiering_2nd')==null){
            $islow=1;
         }else{
            $islow=null;
         }
