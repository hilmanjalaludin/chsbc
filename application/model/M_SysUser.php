...
public function _getOwnUserGroup( $Level = null, $arr_active = null )
{
    $_conds = array();
	$this->db->reset_select();
	$this->db->select('b.UserId, b.id,b.full_name,a.id as UserLevelId');
	...
	public function _getUserGroupBySPV( $Level = null, $arr_active = null )
{
    $_conds = array();
	$out = new EUI_Object(_get_all_request());
	$this->db->reset_select();
	...
	if(is_array($out->get_value('CampaignSPVList'))) {
		$this->db->where_in('b.spv_id',$out->get_value('CampaignSPVList'));
		$this->db->where_not_in('b.handling_type',array(
			USER_ROOT,
			USER_ADMIN,
			USER_MANAGER,
