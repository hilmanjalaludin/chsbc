...
class M_MgtLockCustomer extends EUI_Model
{
    ...
    //     if ($out->get_value('cmb_lock_type') != 3) {
    ...
    //             $this->db->reset_write();
    //             $this->db->set("lock_tmr_id", $val);
    //             $this->db->set("id_lock_type", '1');
    //             $this->db->set("lock_parameter", $out->get_value('lock_two_param'));
				// $this->db->set("type", 1);
    //             $this->db->insert("t_gn_lock_filter");
    ...
    //                 $this->db->set("lock_tmr_id", $val);
    //                 $this->db->set("id_lock_type", '2');
    //                 $this->db->set("lock_parameter", $out->get_value('lock_two_call_result_id'));
				// 	$this->db->set("type", 1);
    //                 $this->db->insert("t_gn_lock_filter");
    ...
    //     }
        ...
        //buka edit irul 20 Februari 2020
        if ($out->get_value('cmb_lock_type') != 3) {
            foreach ($arr_user as $key => $val) {
                $this->db->reset_write();
                $this->db->set("lock_tmr_id", $val);
            ...
            foreach ($arr_user as $key => $val) {
                ...
                $this->db->select('*', FALSE);
                $this->db->from('t_gn_lock_filter a ');
                $this->db->where('a.lock_tmr_id', $val);
                $rs = $this->db->get();
                if( $rs->num_rows() > 0 ) {
                    $this->db->reset_write();
                    $this->db->set("lock_parameter", $out->get_value('lock_two_param'));
                    $this->db->where("id_lock_type", 1);
                    $this->db->where("lock_tmr_id", $val);
                    $this->db->update("t_gn_lock_filter");

                    $this->db->reset_write();
                    $this->db->set("lock_parameter", $out->get_value('lock_two_call_result_id'));
                    $this->db->where("id_lock_type", 2);
                    $this->db->where("lock_tmr_id", $val);
                    $this->db->update("t_gn_lock_filter");
                    $counter++;
                }
        ...
        //tutup edit irul 20 Februari 2020
