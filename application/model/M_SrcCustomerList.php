...
function _get_content()
{
	...
	if (in_array(_get_session('HandlingType'), array(USER_AGENT_OUTBOUND, USER_AGENT_INBOUND) )) {
		$this->EUI_Page->_setArraySelect(array(
			...
			"resp.DisagreeDesc as DisagreeDesc" => array("DisagreeDesc", "Disagree Reason"),
	...
	$this->EUI_Page->_setJoin("t_gn_ver_result g "," a.CustomerId=g.cust_id", "LEFT");
	$this->EUI_Page->_setJoin("t_lk_callreason f "," a.CallReasonId = f.CallReasonId", "LEFT");
	$this->EUI_Page->_setJoin("t_lk_disagree resp "," resp.DisagreeId = a.DisagreeId", "LEFT");
...
function _get_auto_caller_data() 
{
   ...
   $this->getWhereLock();
	...
	// $this->EUI_Page->_setAnd("b.AssignBlock", 0, TRUE);
	$this->EUI_Page->_setAnd("a.Flag_Pds", 0, TRUE);
	$this->EUI_Page->_setAnd("d.CampaignStatusFlag", 1, TRUE);
	$this->EUI_Page->_setAnd("(g.ver_result <> 2 or g.ver_result is null)", FALSE);
...
// -------- map array phone ---------------
 ...
 $arr_map_phone = array(
  ...
  public function _getPhoneCustomer2( $CustomerId=0, $flags = 0  )
{
	$arr_select_contact = array();
...
// -------- map array phone ---------------
 ...
 $arr_map_phone = array(
	...
	if ($data->CustomerHomePhoneNum!='') {
		$home=$data->CustomerHomePhoneNum;
		$mask_home= " Home - "._setMasking($data->CustomerHomePhoneNum);
	}

	if($data->CustomerWorkPhoneNum!=''){
		$work=$data->CustomerWorkPhoneNum;
		$mask_office="Office - "._setMasking($data->CustomerWorkPhoneNum);
	}

	if ($data->CustomerMobilePhoneNum!='') {
		$mobile=$data->CustomerMobilePhoneNum;
		$mask_mobile="Mobile - "._setMasking($data->CustomerMobilePhoneNum);
	}
	
	 
	// var_dump($home,$work,$mobile,$mask_home,$mask_office,$mask_mobile);die();
	$arr=array(
		'val_home'=>$home,
		'val_work'=>$work,
		'val_mobile'=>$mobile,
		'mask_home'=>$mask_home,
		'mask_office'=>$mask_office,
...
{
	if ($this->Row_Lock[0]['type'] == 1 && $this->Row_Lock[1]['type'] == 1) {
		
		$this->EUI_Page->_setAnd( sprintf("( a.".$this->Row_Lock[0]['lock_column']." NOT IN ('%s') OR a.".$this->Row_Lock[1]['lock_column']." NOT IN ('%s'))", implode("','", explode(",",$this->Row_Lock[0]['lock_parameter'])), implode("','", explode(",",$this->Row_Lock[1]['lock_parameter']))));
