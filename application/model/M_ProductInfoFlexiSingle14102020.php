    ...
    $sql = "SELECT *,c.tnc FROM t_gn_attr_flexi_single a
    left JOIN t_gn_frm_flexi_single c ON c.CustomerId = a.CustomerId
    WHERE a.CustomerId = '".$_cust_id."'";
    ...
    // var_dump($obcustomer);die();
        if($var_teiring==100 || $var_teiring==null){
            $islow=1;
         }else{
            $islow=null;
         }
    ...
    $this->db->set("CreateBy" ,$obUsr->get_value('UserId','intval'));
    $this->db->set("DLFlage" ,0);
    $this->db->set("AddressVerif" ,$out->get_value('addressVerif'));
    $this->db->set("NewBillingAddress" ,$NewBillingAddress);
