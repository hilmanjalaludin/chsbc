...
class M_SrcCustomerIncoming extends EUI_Model
{
...
// ------------------------- defualt filter ------------------------	
	...
	// $this->EUI_Page->_setAnd("a.expired_date > ".date('Y-m-d'), FALSE);
	$this->EUI_Page->_setAnd("f.CallReasonEvent", 1, TRUE);
	$this->EUI_Page->_setAnd("(e.ApproveEskalasi = 1 or e.HoldFlag = 1)", FALSE);
...
// ------------------- post filter  --------------------------------------------------------------	
	...
	$this->EUI_Page->_setLikeCache("a.Recsource", "src_customer_recsources", true);
...
// ---------- customize filter ------------------------
	...
	$this->EUI_Page->_setArraySelect(array(
		...
		"e.AproveName as AproveName" => array("AproveName", "QA Status"),
 ...
 */ 
 ...
 public function _getDetailCustomer($CustomerId=null)
 {
	...
	$this -> db -> select('a.*, b.Gender, c.Salutation, d.CampaignName, d.CampaignNumber, e.CallReasonDesc, e.CallReasonCategoryId, f.TableDetail, f.ViewVerification, f.ViewProductInfo ,f.ViewCdd', FALSE);
...
// -------- map array phone ---------------
 ...
 $arr_map_phone = array(
		'CustomerHomePhoneNum' => array('Phone 1', $this->Event), 
		'CustomerMobilePhoneNum' => array('Phone 2',$this->Event), 
		'CustomerWorkPhoneNum' => array('Phone 3', $this->Event),
