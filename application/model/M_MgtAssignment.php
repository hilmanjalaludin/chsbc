...
public function _select_page_distribute( $out  = null, $field = null )
{
 	...
 	// commit b73f81a1db5023ca9080ec9a0c3398daf3ef38f2
 	// $this->db->join("t_lk_callreason u "," u.CallReasonCategoryId=e.CallReasonCategoryId ","LEFT");
 	// commit b73f81a1db5023ca9080ec9a0c3398daf3ef38f2
 	$this->db->join("t_lk_disagree o"," b.DisagreeId=o.DisagreeId ","LEFT");
 	$this->db->where("a.AssignBlock", 0);
 	$this->db->where("b.expired_date >= {$conds2}");
 	$this->db->where("b.campaignid != 9");
 	// $this->db->where("b.CallReasonId =1");
 	...
 	// ---------------- set call result ----------------------------------
  	if( _get_have_post('dis_call_reason')){
  	 ...
  	 if( _get_have_post('disagree3')){
		if($out->get_value('disagree3')!='Choose'){
			$this->db->where_in("b.DisagreeId", $out->get_array_value('disagree3','intval') );
		}
...
// commit b73f81a1db5023ca9080ec9a0c3398daf3ef38f2
  // 	 if( _get_have_post('acc_status')) {
		// $this->db->where(" u.CallReasonCategoryId", $out->get_value('acc_status') );
  // 	}
// commit b73f81a1db5023ca9080ec9a0c3398daf3ef38f2
 ...
 public function _select_page_transfer( $out  = null, $field = null )
{
 ...
 $this->db->join("t_gn_campaign c "," b.CampaignId=c.CampaignId ","LEFT");
 //edit 11/2/2020
 $this->db->join("t_lk_disagree u "," b.DisagreeId=u.DisagreeId ","LEFT");
 //
...
// Commit 6ee2a75651a3adaba17e3a1d1d718824a76097ff
//  if( _get_have_post('disagree2')){
// 	if($out->get_value('disagree2')!='Choose'){
// 	  $this->db->where_in("b.DisagreeId", $out->get_value('disagree2') );
// 		}
// }
  ...
  if( _get_have_post('disagree2')){
	if($out->get_value('disagree2')!='Choose'){
		$this->db->where_in("b.DisagreeId", $out->get_array_value('disagree2','intval') );
	}
...
//update 11/02/2020
  if( _get_have_post('disagree')){
	  if($out->get_value('disagree')!='Choose'){
		$this->db->where_in("b.DisagreeId", $out->get_array_value('disagree','intval') );
	  }
...
public function _select_page_distributePDS( $out  = null, $field = null )
{
	...
	// assign by checked
	if( _get_have_post('AssignId')){
		$this->db->where_in("a.AssignId", $out->get_array_value('AssignId') );
  	}
	
	// assignspv by checked
	if( _get_have_post('dis_spv')){
		$this->db->where_in("a.AssignSPV", $out->get_array_value('dis_spv') );
  	}
...
function checkPDSRunningCounter($campaignId){
	$this->db->reset_select();
	// $this->db->select("a.PDSRunningCount", FALSE);
	$this->db->select("a.StatusRunning", FALSE);
...
function getCustomerPDSCampaign($campaignId){
	$this->db->reset_select();
	$this->db->select("a.CustomerPdsId,a.CustomerId", FALSE);
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ){
			$data[$rows['CustomerPdsId']] = $rows['CustomerId'];
		}
...
public function _ClearPDSCampaign($campaignId){
	$checkPDSRunningCounter = $this->checkPDSRunningCounter($campaignId);
	
	if(!$checkPDSRunningCounter){
		return false;
	}else{
		$getCustomerList = $this->getCustomerPDSCampaign($campaignId);
		if(count($getCustomerList)){
			$i=0;
			foreach($getCustomerList as $CustomerPdsId => $CustomerId){
				$this->db->reset_write();
				$this->db->set("CustomerPdsId", $CustomerPdsId);
				$this->db->set("CustomerId", $CustomerId);
				$this->db->set("CampaignId", $campaignId);
				$this->db->set("ClearLogDate", date("Y-m-d H:i:s"));
				$this->db->insert("t_gn_customer_pds_clear_log");
				if($this->db->insert_id()){
					$this->setPdsCustomerFlag($CustomerId);
					$i++;
				}
			}
		}
		
		if(count($getCustomerList)==$i){
			$this->db->reset_write();
			$this->db->where("CampaignId", $CampaignId);
			$this->db->delete("t_gn_customer_pds");
...
public function _AssignPdsLog($logdata=null, $out=null){
	$i=0;
	if(is_object($logdata)){
		$this->db->reset_write();
		// $this->db->set("Id", $logdata->);
		$this->db->set("CustomerId", $logdata->get_value('CustomerId'));
		$this->db->set("CurCallReasonId", $logdata->get_value('CallReasonId'));
		$this->db->set("CurSellerId", ($logdata->get_value('AssignSelerId')?$logdata->get_value('AssignSelerId'):0));
		$this->db->set("AssignById", _get_session('UserId'));
		$this->db->set("AssignToCampaign", $out->get_value('dis_campaign_name_pds'));
		$this->db->set("AssignDate", date("Y-m-d H:i:s"));
		$this->db->set("LastCallTs", null);
		$this->db->set("LastCallStatusId", 0);
		$this->db->set("LastCallCategoryId", 0);
		$this->db->set("CallCount", 0);
		$this->db->set("AssignSession",$logdata->get_value('AssignSession'));
		$this->db->insert("t_gn_customer_pds_asgmt_log");
		if($this->db->insert_id()){
			$i++;
		}
	}
