...
class M_VerifikasiA extends EUI_Model
{
	...
	function _save_ver_activity($param, $type) {
		...
		if ($type == 'ask_1') {
			...
			$val = $rowCustomer[0]->CustomerWorkPhoneNum;
			$inp = $param['no_telp'];
			...
			$homeNumber = substr($rowCustomer[0]->CustomerHomePhoneNum,0,3);
			$workNumber = substr($rowCustomer[0]->CustomerWorkPhoneNum,0,3);
			if ($homeNumber == '021' && $rowCustomer[0]->CustomerHomePhoneNum != '') {
				if ($homeNumberCut == $param['no_telp']) {
					$val = 'Home|'.$homeNumberCut;
					$inp = 'Home|'.$param['no_telp'];
					// $val = $homeNumberCut.'|'.$workNumberCut;
				} else if ($workNumberCut == $param['no_telp']) {
					$val = 'Work|'.$workNumberCut;
					$inp = 'Work|'.$param['no_telp'];
				}
			} else if ($workNumber == '021' && $rowCustomer[0]->CustomerWorkPhoneNum != '') {
				if ($homeNumberCut == $param['no_telp']) {
					$val = 'Home|'.$homeNumberCut;
					$inp = 'Home|'.$param['no_telp'];
					// $val = $homeNumberCut.'|'.$workNumberCut;
				...
				if ($rowCustomer[0]->CustomerHomePhoneNum == $param['no_telp']) {
					$val = 'Home|'.$rowCustomer[0]->CustomerHomePhoneNum;
					$inp = 'Home|'.$param['no_telp'];
				} else if ($rowCustomer[0]->CustomerWorkPhoneNum == $param['no_telp']) {
