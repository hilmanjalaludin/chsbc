 ...
 public function _select_report_manager( $Manger = 0 ) 
{
 ...
 {
	$sql = "select a.UserId, a.id  from tms_agent a 
			where a.handling_type = 2 and a.UserId='$gUserId' 
			and a.user_state=1"; 
 }
 ...
 {
	$sql = "select a.UserId, a.id  from tms_agent a  where a.handling_type = 2 and a.UserId IN (
 ...
 function _select_report_atm_by_manager( $ManagerId = 0 )
{
 ...
 $this->db->where_in("a.mgr_id",  $ManagerId);
 $this->db->where("a.user_state", 1);
 
 
// --------- handle by login  ------------- 
  if( in_array($gHandle, 
   array(USER_SUPERVISOR,USER_ROOT) ) ) {
 ...
 public function _select_report_type()
{
 ...
 $this->report_type = array(
	'filter_campaign_group_date' => 'Summary By Recsource',
	'filter_campaign_group_mgr' => 'Summary by Recsource per AM',
	'filter_campaign_group_spv' => 'Summary by Recsource per SPV',
	'filter_campaign_group_agent' => 'Summary by Recsource per Agent'
