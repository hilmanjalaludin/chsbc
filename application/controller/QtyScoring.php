 ...
 */
 ...
 public function QtyScoring() 
 {
	parent::__construct();
		...
		$this->load->model(array('M_SetResultQuality', 'M_SrcCustomerList'));
 }
...
{
	#var_dump($this->M_SetResultQuality->_getQualityResult());die();
...
{
	$CustomerId = '890859';
	var_dump($this->M_SrcCustomerList->_getPhoneCustomer($CustomerId));die();
	$UI = array
	(
...
//   print_r(_get_all_request());
  ...
  if ( is_array($getAllPost) ) {
		...
		**/
		...
		#print_r('asd',$this->{base_class_model($this)}->_setSaveScoreQuality($getAllDataPost));
