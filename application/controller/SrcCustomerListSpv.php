...
class SrcCustomerListSpv extends EUI_Controller
{
...
{
	parent::__construct();
	$this->load->model(array('M_SrcCustomerListSpv', 'M_SetCallResult','M_SetProduct', 'M_SetCampaign','M_SetResultCategory', 'M_Combo', 'M_SysUser', 'M_MgtLockCustomer' ));
 ...
 */
 ...
 function index()
 {
	if( $this ->EUI_Session ->_have_get_session('UserId') )
	{
		...
		$this->load ->view('src_customer_listspv/view_customer_spv_nav',array(
			'page' => $this->M_SrcCustomerListSpv->_get_default(),
			'recs' => $this->M_MgtLockCustomer->_getUserRecsource(_get_session('UserId')),
			'callreason' => $this->M_SrcCustomerListSpv->_getCallReason()
		));
	}	
 }
...
function Content()
 {
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		$_EUI['page'] = $this ->M_SrcCustomerListSpv ->_get_resource();    // load content data by pages 
		$_EUI['num']  = $this ->M_SrcCustomerListSpv ->_get_page_number(); // load content data by pages 
		...
		{
			$this -> load -> view('src_customer_listspv/view_customer_spv_list',$_EUI);
		}
	}
 }
 /*
