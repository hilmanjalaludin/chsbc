 ...
 */
  ...
  public function EscoringQualityDetail()
{
...
//---------- load object by instance ------------
    ...
    $selectCoaching = $this->db->query(
			"SELECT * FROM t_gn_score_result a WHERE a.CustomerId='$CustomerId'"
		)->row_array();
    if($selectCoaching['notes']){
	...
	// var_dump($objectCustomer);
	// die;
	#var_dump(ApprovalPoint());die();
// --------- load view data ------------------------------------
	 $this->load->view('mod_e_scoring/qty_approval_interest/view_quality_detail',array 
	(
