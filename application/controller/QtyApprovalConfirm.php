...
class QtyApprovalConfirm extends EUI_Controller
{
...
//---------- load object by instance ------------
	...
	$objBenef 	  =& get_class_instance('M_Benefiecery');
	
// --------- load view data ------------------------------------
	 $this->load->view('qty_approval_interest/view_quality_detail',array 
	(
		'Customers' 	   => new EUI_Object( $objCustomer->_getDetailCustomer($CustomerId)), 
		'Policy'  		   => new EUI_Object(PayerInformation($CustomerId)),
		'Payer'			   => new EUI_Object(PayerReady( $CustomerId )),
		'Insured' 		   => $objInsured->_getInsuredById( $CustomerId ),
		'Benefiecery' 	   => $objBenef->_getBeneficiaryCustomerId( $CustomerId ),
...
{
 if(_have_get_session('UserId') && class_exists('M_QtyApprovalConfirm') )
 {
	$this->load->view('qty_approval_confirm/view_approval_confirm_nav',array( 
...
public function Content()
{
  ...
  if(_have_get_session('UserId') ) 
  {
	...
	if( is_array($_EUI) && is_object($_EUI['page']) ) {
		$this -> load -> view('qty_approval_confirm/view_approval_confirm_list',$_EUI);
	 }	
  }	
}
 ...
 public function SaveQualityActivity()
{
 ...
 // --------------------------------------------------------- 
  ...
  $checkScore = $obClass->_checkScoreResult( $out->get_value('CustomerId','intval'));
  
  if( $checkScore == FALSE ){
	$cond = array("success" => 0 , "scoring" => 0);
  ...
  echo json_encode(array("success" => 0 , "scoring" => 0));
}
// ---------------------------------------------------------------------------
/*
 * @ def 		: approval save point 
