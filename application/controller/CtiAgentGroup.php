<?php
class CtiAgentGroup extends EUI_Controller
{


function CtiAgentGroup() 
{
	parent::__construct();
 ...
 */
 ...
 function index()
 {
	if( $this ->EUI_Session -> _have_get_session('UserId') )
	{
		...
		if( is_array($_EUI))
		{
			$this -> load -> view('cti_agent_group/view_agentgroup_nav',$_EUI);
		}	
	}	
 }
...
function content()
 {
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		...
		{
			$this -> load -> view('cti_agent_group/view_agentgroup_list',$_EUI);
		}	
	}	
 }
 ...
 */
 function Add()
 {
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		$this -> load -> view('cti_agent_group/view_agentgroup_add',array('FieldName' =>  $this -> {base_class_model($this)}-> _getFieldName()));
	}
 }
 ...
 */
 ...
 function Edit()
 {
	...
	{
		$UI = array(
				'Reason' => $this -> {base_class_model($this)}->_getReasonTypeData($this -> URI->_get_post('Id')), 
				'FieldName' =>  $this -> {base_class_model($this)}-> _getFieldName()
			);
		
		$this -> load -> view('cti_agent_group/view_agentgroup_edit',$UI);
	}
