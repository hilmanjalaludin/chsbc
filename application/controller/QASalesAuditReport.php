<?php
	class QASalesAuditReport extends EUI_Controller
	{
		function QASalesAuditReport()
		{
			parent::__construct();
		...
		function index()
		{
			...
			$this->load->view('rpt_QA_sales_audit/rpt_qa_sales_audit_nav',$UI);
		}
		...
		public function GroupFilterBy()
		{
			...
			{
				__(form()->combo('spvname','select long', $this->M_QASalesAuditReport->_getAtmBaru(), null, array("change" => "Ext.DOM.ListTL(this)") ));
			}
			else
			{
		...
		public function GroupFilterTL($spv)
		{
			if($this -> URI -> _get_have_post('FilterId'))
			{
				$d  = $this->M_QASalesAuditReport->_getSPVBaru($_REQUEST['FilterId']);
		...
		public function GroupFilterTSR($spv)
		{
			if($this -> URI -> _get_have_post('FilterId'))
			{
				$a = $this->M_QASalesAuditReport->_getTSRBaru($_REQUEST['FilterId']);
		...
		public function SummaryHTML()
		{
			...
			$this -> load -> view("rpt_QA_sales_audit/rpt_QA_sales_audit_show", $UI);
		}
		...
		public function SummaryExcel()
		{
			...
			$this -> load -> view("rpt_QA_sales_audit/rpt_qa_sales_audit_download", $UI);
		}
		...
		public function _CAMPAIGN()
		{
			...
			{
				$_reqs[$v] = $this ->M_QASalesAuditReport->filter_by($v); 	
			}
			return $_reqs; 	
		}
	}
