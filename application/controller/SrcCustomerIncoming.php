...
class SrcCustomerIncoming extends EUI_Controller
{
...
{
	parent::__construct();
	$this->load->model(array('M_SrcCustomerIncoming', 'M_SetCallResult','M_SetProduct', 'M_SetCampaign','M_SetResultCategory', 'M_Combo', 'M_SysUser' ));
 ...
 */
 ...
 function index()
 {
	if( $this ->EUI_Session ->_have_get_session('UserId') )
	{
		...
		$this->load ->view('src_customer_incoming/view_customer_nav',array(
			'page' => $this->M_SrcCustomerIncoming->_get_default()
		));
	}	
 }
...
function Content()
 {
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		$_EUI['page'] = $this ->M_SrcCustomerIncoming ->_get_resource();    // load content data by pages 
		$_EUI['num']  = $this ->M_SrcCustomerIncoming ->_get_page_number(); // load content data by pages 
		
		// sent to view data 
		
		if( is_array($_EUI) && is_object($_EUI['page']) )  
		{
			$this -> load -> view('src_customer_incoming/view_customer_list',$_EUI);
		}
	}
 }
 /*
 ...
 public function ContactDetail()
{
	 ...
	 if(  _get_have_post('CustomerId') )
	{
		...
		{
			$this->load->view('mod_reconfirm_detail/view_contact_main_detail', array(
