...
// ----------------
 ...
 public function SelectUserBySess(){
	$arrUser = array();
	$User =& get_class_instance('M_SysUser');
	$out = new EUI_Object(_get_all_request());
	
	if( _get_session('UserId') ){
		$arrUser =& $User->_getUserGroupWithMe($out->get_value('handling'),1);
	}
	...
	// exit();
	if( $out->get_value('type') == 'dropdown' ){
		echo form()->combo($out->get_value('id'),"select tolong",$arrUser, first($OnSelect),array('change'=>'Ext.DOM.SelectTmrBySpv(this.value);'));
	}

	if( $out->get_value('type') == 'listboxes' ) {
		echo form()->listCombo($out->get_value('id'),"select tolong", $arrUser,$OnSelect, array('change'=>'Ext.DOM.SelectTmrBySpv();'), array("height" => "200px","dwidth" => "100%"));
	}
	
	// echo _get_session('HandlingType');
 }
 
 public function SelectUserBySpvs(){
	$arrUser = array();
	$User =& get_class_instance('M_SysUser');
	$out = new EUI_Object(_get_all_request());
	
	if( _get_session('UserId') ){
		$arrUser =& $User->_getUserGroupBySPV($out->get_value('handling'),1);
	}
	...
	// exit();
	if( $out->get_value('type') == 'dropdown' ){
		echo form()->combo($out->get_value('id'),"select tolong",$arrUser, first($OnSelect));
	}else if( $out->get_value('type') == 'listboxes' ) {
		echo form()->listCombo($out->get_value('id'),"select tolong", $arrUser,$OnSelect, null, array("height" => "200px","dwidth" => "100%"));
	}
 }
...
function filtered_CallResult()
{
	...
	$OnSelect = $out->get_array_value('select');
		if( $out->get_value('type') == 'dropdown' ) {
			echo form()->combo($out->get_value('id'),"select long",$_conds, first($OnSelect));
		}	
		else if( $out->get_value('type') == 'listboxes' ) 
		{
			echo form()->listCombo($out->get_value('id'),"select long", $_conds,$OnSelect, null, array("height" => "200px","dwidth" => "100%"));		
		}
}

function filtered_CallResultPDS()
{
	$_conds = array();
	$out = new EUI_Object(_get_all_request() );
	$CallResult = $this -> M_SetCallResult->_getCallReasonIdPds(null);
	$Sale = $this -> M_SetCallResult->_getInterestSale();
...
public function ClearPDSCampaign(){
	$out = new EUI_Object( _get_all_request() );
	$cond = array('success' => 0, 'msg' => 'fail');
	...
	$class_MgtAssign =& get_class_instance(base_class_model($this));
	$clearPDS = $class_MgtAssign->_ClearPDSCampaign($out->get_value('campaignId'));
	
	if(!$clearPDS){
		$cond = array('success' => 0, 'msg' => 'pds is not clear');
	}else{
		$cond = array('success' => 1, 'msg' => 'pds is clear');
	}
	
	echo json_encode( $cond );
	// print_r($out->get_value('campaignId'));
}
