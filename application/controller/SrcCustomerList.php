...
{
	parent::__construct();
	$this->load->model(array('M_SrcCustomerList', 'M_SetCallResult','M_SetProduct', 'M_SetCampaign','M_SetResultCategory', 'M_Combo', 'M_SysUser', 'M_MgtLockCustomer' ));
...
function ViewAddReferral()
{
	...
	if( is_array($Data) ) {
		$this->load->view('mod_contact_detail/view_add_referral',$Data);
	}
}

// get digit bank
public function getDigitBank ( $idBank = 0 ) {

	$callback = array("minlength"=>0,"maxlength"=>15);

	if ( $idBank != 0 ) {
		$getBank = "SELECT * FROM t_lk_bank a WHERE a.BankId='$idBank'";
		$SelectBank = $this->db->query($getBank);
		if ( $SelectBank == true AND $SelectBank->num_rows() > 0 ) {
			$SB = $SelectBank->row();
			$mindigit = $SB->MinDigit;
			$maxdigit = $SB->Digit;
			$callback = array("minlength"=>$mindigit,"maxlength"=>$maxdigit);
		} 
	} 
 ...
 function index()
 {
	if( $this ->EUI_Session ->_have_get_session('UserId') )
	{
		...
		$this->load ->view('src_customer_list/view_customer_nav',array(
			'page' => $this->M_SrcCustomerList->_get_default(),
...
function Content()
 {
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		$_EUI['page'] = $this ->M_SrcCustomerList ->_get_resource();    // load content data by pages 
		$_EUI['num']  = $this ->M_SrcCustomerList ->_get_page_number(); // load content data by pages 
		
		// sent to view data 
		
		if( is_array($_EUI) && is_object($_EUI['page']) )  
		{
			$this -> load -> view('src_customer_list/view_customer_list',$_EUI);
		}
	}
 }
 /*
 ...
 public function ContactDetail()
{
	 ...
	 if(  _get_have_post('CustomerId') )
	{
		...
		if($second_product = $out->_getDetailSecondProduct($var->get_value('CustomerId'))){
			...
			if( $arr_ouput = $out->_getDetailCustomer( $var->get_value('CustomerId') ) ){
				// var_dump($arr_ouput['TableDetail']);
				$this->load->view('mod_contact_detail/view_contact_main_detail', array(
					'Detail' 		=> new EUI_Object( $arr_ouput ),
					'Detail_attr' 	=> $out->_getDetailAttr($arr_ouput['TableDetail'], $var->get_value('CustomerId')),
					'ver_res_stat'	=> $verif_stat,
...
public function SetFollowup()
{
	 ...
	 // -------- set follow up ---------------------------------
	 #var_dump(new EUI_Object( _get_all_request() ));die();
...
// function order untuk process call auto start  dari 
// sessiion 
...
function Order(){
 ...
 if( is_object( $this->std ) ){
	$this->result_data = $this->std->_get_auto_caller_data();
 }
 
 // return on json data 
 if( function_exists( 'json_encode' )  ){
	$json_data  = array( 'data' => $this->result_data); 	 
	printf("%s", json_encode( $json_data ) );
	return false;
 }
...
public function parameter_recsource()
{
	...
	if( _get_session('UserId') )
	{
		...
		if( _get_have_post('recsource_param_name') )
		{
			$campaign = $out->get_array_value('recsource_param_name');
			
		}
		
		if( _get_have_post('src_param_name') )
		{
			$campaign = $out->get_array_value('src_param_name');
		}
		print_r($campaign);
...
// tambahan untuk process filter di recsource
// http://10.10.10.62/SrcCustomerList/FilterRecData/?keyword=a&_=1503398269660
...
// result dropdown entitas.	
	printf("%s", form()->combo('src_customer_recsource', 'select long cz-autocomplete', $result_array, $result_value, null, array('multiple'=>'multiple' ) ) );
	return false;
}
...
public function filtered_recsource()
{
	...
	if( _get_session('UserId') )
	{
		...
		if( _get_have_post('src_campaign_name') )
		{
			...
			$istext = $out->get_value('src_customer_keyword');
		}
...
public function cekStatusNoAutoDial()
{
 	$arr_response = array('success' => 0 );
 	if( !_get_have_post('CustomerId') OR !_have_get_session('UserId') )
 	{
 	...
 	$cond = $this->{base_class_model($this)}->_cekStatusNoAutoDial(new EUI_Object( _get_all_request() ));
 	#echo json_encode( array('debug' => $cond) ); die();
