<?php
class NewDashboard extends EUI_Controller
{
	var $_temp = array();
	...
	function index()
	{
		$this->load->view('new_dashboard/new_dashboard_nav',array(
			'type' => $this->M_NewDashboard->_get_dashboard_type()
		));
	}
	...
	function handle_type_mgr()
	{
		switch( $this->URI->_get_post('type') )
		{
			case 'detail-per-tso' :
				...
				{
					case USER_ROOT : // root
						$_user = $this->M_NewDashboard->_get_combo_user_query(array(
	...
	function ShowDashboard()
	{
		if( isset($this->_temp[$this->URI->_get_post('dsb_type')]) )
		{
			...
			// echo $_conn;
			$this->load->view('new_dashboard/new_dashboard_show',array(
				'datas' => $this->M_NewDashboard->{$_conn['model']}($this->URI->_get_all_request()),
				'touch' => $this->M_NewDashboard->{$_conn['solicited']}($this->URI->_get_all_request()),
				'times' => $this->M_NewDashboard->_get_jam_current($this->URI->_get_all_request()),
				'loops' => ($this->URI->_get_have_post($_conn['loop'])?$this->URI->_get_post($_conn['loop']):null),
				'users' => $this->M_NewDashboard->_get_combo_user_query(),
