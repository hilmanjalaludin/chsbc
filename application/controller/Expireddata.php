...
class Expireddata extends EUI_Controller
{
...
{
	parent::__construct();
	$this->load->model(array('M_Expireddata', 'M_SetCallResult','M_SetProduct', 'M_SetCampaign','M_SetResultCategory', 'M_Combo', 'M_SysUser', 'M_MgtLockCustomer' ));
...
function ViewAddReferral()
{
	...
	if( is_array($Data) ) {
		$this->load->view('mod_expired_detail/view_add_referral_spv',$Data);
	}
}
...
function setCallResult() 
{
	$_result = array( "setCallResult" => $this -> getCallResult( _get_post('CategoryId') ) );
	 if( is_array($_result)) 
	{
		$this -> load ->view('mod_expired_detail/view_call_result_spv',$_result);
	}
}
 ...
 */
 ...
 function index()
 {
	if( $this ->EUI_Session ->_have_get_session('UserId') )
	{
		...
		$this->load ->view('expired_data_list/view_expired_nav',array(
			'page' => $this->M_Expireddata->_get_default(),
			'recs' => $this->M_MgtLockCustomer->_getUserRecsource(_get_session('UserId')),
			'callreason' => $this->M_Expireddata->_getCallReason()
		));
	}	
 }
...
function Content()
 {
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		$_EUI['page'] = $this ->M_Expireddata ->_get_resource();    // load content data by pages 
		$_EUI['num']  = $this ->M_Expireddata ->_get_page_number(); // load content data by pages 
		
		// sent to view data 
		
		if( is_array($_EUI) && is_object($_EUI['page']) )  
		{
			$this -> load -> view('expired_data_list/view_expired_list',$_EUI);
		}
	}
 }
 /*
 ...
 public function ContactDetail()
{
	 ...
	 if(  _get_have_post('CustomerId') )
	{
		...
		{
			$this->load->view('mod_expired_detail/view_contact_main_detail_spv', array(
