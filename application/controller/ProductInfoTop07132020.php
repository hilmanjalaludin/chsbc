		...
		if( is_array($loans) ){
			...
			foreach( $loans as $num => $rows ){
				...
				echo	"<tr bgcolor=\"{$back_color}\" class=\"onselect\" height=\"35\">".
						...
						"<td class=\"content-first\" nowrap>".form()->radio( "tenor_", "content-first", $row->get_value('Tenor',$arr_function['Tenor']), array('click'=>'alert(this.value)'))."</td>";
					...
					foreach( array_keys($arr_header) as $k => $fields ){
						if(strtolower($fields) == 'tenor'){
							...
							if(strtolower($fields) == 'rate'){
								...
								$long = number_format($loan, 4, '.', ',');
							}
						}
						echo  "<td align='right' class=\"".$arr_class[$fields]." ui-widget-unselect-order ".$arr_align[$fields]."\" ".$arr_wrap[$fields].">".$long."</td>";
					}
				}else{
					foreach( array_keys($arr_header) as $k => $fields ){
						if(strtolower($fields) == 'tenor'){
							...
							$numbers = number_format($row->get_value($fields,$arr_function[$fields]), 4, '.', ',');
						}
						echo  "<td align='right' class=\"".$arr_class[$fields]." ui-widget-unselect-order ".$arr_align[$fields]."\" ".$arr_wrap[$fields].">".$numbers."</td>";
					}
				}
				echo "</tr>";
				$no++;
