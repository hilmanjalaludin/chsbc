<?php
	class CampaignEffectiveness extends EUI_Controller
	{
	
		function CampaignEffectiveness()
		{
			parent::__construct();
			$this -> load -> model(array(base_class_model($this),'M_CampaignEffectiveness'));
		}
		...
		function index()
		{
			$UI = array
			(
				'filter_by'		=> $this -> M_CampaignEffectiveness -> FilterBy(),
				'mode_by'		=> $this -> M_CampaignEffectiveness -> ModeBy()
			);
		...
		public function getCampaign()
		{
			...
			{
				$_conds = $this -> M_CampaignEffectiveness -> filter_by($this -> URI->_get_post('FilterId'));
		...
		public function _CAMPAIGN()
		{
			...
			{
				$_reqs[$v] = $this ->M_CampaignEffectiveness->filter_by($v); 	
			}
			return $_reqs; 	
		}
