<?php
/*
 * @ parameter : SetCampaignLinked 
 * -----------------------------------------------------------
 * -----------------------------------------------------------
 * -----------------------------------------------------------
 */
 
class SetCampaignLinked extends EUI_Controller
{

 function SetCampaignLinked() 
 {
	parent::__construct();
 ...
 */
 ...
 function index()
 {
	if( $this ->EUI_Session -> _have_get_session('UserId') )
	{
		...
		if( is_array($_EUI))
		{
			$this -> load -> view('mod_linked_view/view_linked_nav',$_EUI);
		}	
	}	
 }
...
function content()
 {
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		...
		{
			$this -> load -> view('mod_linked_view/view_linked_list',$_EUI);
		}	
	}	
 }
 ...
 */
 ...
 function Add()
 {
	...
	if( $this -> EUI_Session -> _have_get_session('UserId') )
	{
		$this -> load -> view('mod_linked_view/view_linked_add',
			array
			(
				'FieldName' => $this -> {base_class_model($this)}-> _getFieldName(),
				'HTML' 		=> $this -> {base_class_model($this)}-> _getComponent(),
				'MetaJoin'  => $this -> {base_class_model($this)}->_getMetaJoin()
			)
		);
	}
 }
 ...
 */
 ...
 function Edit()
 {
	...
	{
		$UI = array(
				'IVR' 		=> $this -> {base_class_model($this)}->_getData($this -> URI->_get_post('Id')), 
				'FieldName' => $this -> {base_class_model($this)}-> _getFieldName(),
				'HTML' 		=> $this -> {base_class_model($this)}->_getComponent(),
				'MetaJoin'  => $this -> {base_class_model($this)}->_getMetaJoin() 
			);
		
		$this -> load -> view('mod_linked_view/view_linked_edit',$UI);
	}
