...
class Escoring extends EUI_Controller{
...
{
	parent::__construct();
	$this->load->model(array('M_escoring')); // load model user
...
function index() 
{
	...
	{
		$page['page'] = $this -> M_escoring -> get_default();
		$page['AOC'] = $this->_getAgentBySpv();
		$this -> load -> view("mod_e_scoring/view_coach_nav", $page);
	}
	//echo "tes";
}
...
function content()
{
	$page['page'] = $this -> M_escoring -> get_resource_query(); // load content data by pages 
	$page['num']  = $this -> M_escoring -> get_page_number(); 	// load content data by pages 
	if( is_array($page) && is_object($page['page']) ) 
	{
		$this -> load -> view("mod_e_scoring/view_coach_list", $page );
	}	
}
...
function UserCapacity()
{
	...
	$Capacity = $this -> M_escoring -> _getUserCapacity(1);
...
public function EditCoachAgent () {
	...
	$note                = $RequestData->get_value('note');
    // var_dump($RequestData);
    // die;
	if ( _have_get_session("UserId") ) {
		// select coaching 
		$selectCoaching = $this->db->query(
			"SELECT * FROM t_gn_score_result a WHERE a.CustomerId='$CoachingId'"
		);
		if ( $selectCoaching->num_rows() > 0 ) {
            $data=$selectCoaching->row_array();  
            if($data['notes']){
            	$status_send = array( "success" => 0 , "message" => "Failed update!" );
            }else{
            $this->db->set("Development_Required" , 'yes');
			$this->db->set("notes" , $note);
			$this->db->set("DateCreateTs" , date("Y-m-d H:i:s"));
			$this->db->where("CustomerId" , $CoachingId );
			$this->db->update("t_gn_score_result");
			if ( $this->db->affected_rows() > 0 ) {
				$status_send = array( "success" => 1 , "message" => "Success Input Scoring!" );
			}
           }
...
public function EditCoachSpv () {
	...
	if ( $_getIdCoach != "" ) {
		...
		if ( $selectCoaching_s == true AND $selectCoaching_s->num_rows() > 0 ) {
			...
			$this->load->view( "mod_e_scoring/view_edit_coaching_spv" , $varData );
		} 
	} else {
