 ...
 * @return	string
 */
class U_Flexi_Single extends EUI_Upload
{
		...
		foreach($_set_loan_tiering as $keys => $vals){
			foreach($vals as $key => $val){
				...
				if( $this->db->affected_rows() > 0 ) {
					...
					// insert 5J
					if( strlen(trim($key)) ){
						$rate = $_set_loan_tiering[$keys][$key]['interest'];
						if( strlen(trim($rate)) > 0 AND !array_key_exists($key,$tenors) ) {
							$tenors[$key] = $key;
							$installments= ((5000000/$key) + (5000000*$_set_loan_tiering[$keys][$key]['interest']));
							$this->db->reset_write();
							$this->db->set('CustomerId',$_cust_id);
							$this->db->set('Cardno', ($_set_loan_tiering[$keys][$key]['cardno'] !="" ? $this->replaceCarakter($_set_loan_tiering[$keys][$key]['cardno']):NULL) );
							$this->db->set('CampaignId', $this->_get_campaignId());
							$this->db->set('Tenor', ($key !="" ? $this->replaceCarakter($key):NULL) );
							$this->db->set('LoanAmount','5000000');
							// $this->db->set('Installment',round(str_replace(",",".",$this->replaceCarakter(number_format($installments,0),array(",")))));
							$this->db->set('Installment',round(str_replace(",",".",$installments)));
							$this->db->set('Rate',$rate);
							$this->db->set('UploadDate', date('Y-m-d H:i:s'));
							$this->db->set('Desc_Card','CARD 99');
							$this->db->insert('t_gn_loan_tiering');
							#var_dump( $this->db->last_query() ); die();
						}
					}
				...
				*/
				...
				if( $this->db->affected_rows()>0 )  
				{
					...
					if( !is_null($this->_upload_table_name) ) {
						...
						// ini masuk ke table masing-masing campaign		
						// print_r($values);
						// var_dump($this->_upload_table_name);die();
						$query = $this->db->insert( $this->_upload_table_name, $values );
						//query insert table fs
						// var_dump($this->db->last_query());die();
						if( $this->db->affected_rows() > 0 ) {
