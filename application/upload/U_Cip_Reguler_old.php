		...
		for($i=1; $i <= $jml_tier;) {
			...
			// if( (int)trim(@$loan_tiering[$tenor]) ) {
				...
				foreach ($tier as $key => $value) {
					...
					$_inst     = $_ins.$i; // Installment

					$_ins_fee  = $ins_free[$_rate_no];
					$_ins_fee_ = $_ins_fee.$i; // Free_Interest

					$_admfee   = $admin_fee[$_rate_no];
					$_admfee_  = $_admfee.$i; // AdminFee

					// if( (int)@$loan_tiering[$_tenor] ) {
						$this->db->set('Cardno',@$loan_tiering['cardno']);
						$this->db->set('CampaignId',$this->_get_campaignId());
						$this->db->set('UploadDate', date('Y-m-d H:i:s'));
						$this->db->set('CustomerId',(int)$_cust_id);
						$this->db->set('Tenor',(int)@$loan_tiering[$_tenor]);
						$this->db->set('LoanAmount',(float)@$loan_tiering[$_amount]);
						$this->db->set('AdminFee',(float)@$loan_tiering[$_admfee_]);
						$this->db->set('Rate',@$loan_tiering[$_rate]);
						$this->db->set('Installment',(float)@$loan_tiering[$_inst]);
						$this->db->set('Free_Interest',(float)@$loan_tiering[$_ins_fee_]);
