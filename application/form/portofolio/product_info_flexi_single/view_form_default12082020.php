...
// ----------------------------------------------------------------------------------------
	...
	$(document).ready(function() {
		...
		$("#tnc").change(function() {
			...
			if(value=='NO'){	
				...
				({
						url     : Ext.EventUrl(['SetProductScript','ShowProductScripttncflexisingle']).Apply(), 
						name    : 'WinProduct',
   ...
   Ext.DOM.getVerificationStatus_2nd = function (callback){
		...
		Ext.Ajax({
			url		: Ext.EventUrl(new Array('ProductInfoFlexiSingle', 'getVerificationStatus')).Apply(),
			method	:'POST',
	...
	Ext.DOM.Fallback = function (verification){
		...
		if(verification){
			Ext.DOM.saveProductInfoFlexiSingle();
		}else{
	...
	Ext.DOM.saveProductInfoFlexiSingle = function (){
		...
		// var isVerif = document.getElementById('VerifForm').value;
		// var isVerif = document.getElementById('ver_status_flexi').value;
		// alert(isVerif);
		// return false;
		...
		if( Ext.Cmp('typexsell_2nd').getValue() == '' ){
			...
			if( Ext.Cmp('typexsell_2nd').getValue() == 'YES' ){
				...
				if( Ext.Cmp('vartiering_2nd').getValue()<1){
					...
					window.alert('Benef Name is empty!');
					return false;
				}
				...
				/*else if(lengthaccount.length < 10){
						...
						if($('input[name=key_tenor_2nd]:checked').val()==null){
							...
							// alert(Ext.Cmp('vartiering_2nd').getValue());
							// return false;
							if (r == true) {
									Ext.Ajax({
										url		: Ext.EventUrl(new Array('ProductInfoFlexiSingle', 'saveLoan')).Apply(),
										method	:'POST',
										...
										Ext.Util(fn).proc(function(save){
											if( save.success ) {
												...
												Ext.Cmp('InputForm_2nd').setValue('1');
											}else{
...
<fieldset class="corner" style="margin-bottom:15px;border:0px solid #fff;padding:20px 0px 20px 0px;">
	...
	<div class="ui-widget-form-table" style="margin-top:-5px;">
		...
		<div class="ui-widget-form-row">
			...
			<div class="ui-widget-form-cell text_caption">:</div>
			<div class="ui-widget-form-cell"><?php echo form()->combo("addressVerif_2nd", "select tolong", array('1'=>'Ya','0'=>'Tidak'), $frm['addressVerif'], array("change"=>"Ext.DOM.NewAddress(this.value)"));?></div>
		</div>
		<div class="ui-widget-form-row">
		...
		<div class="ui-widget-form-row" align="Right">
			...
			<div class="ui-widget-form-cell" align="Right">
				...
				<?php if(_get_session('HandlingType')==4 || _get_session('HandlingType')==8){
					// echo form()->button("ButtonUserSave_2nd", "button save", lang('Save Prod. Info Flexi'), array('click' => 'Ext.DOM.saveProductInfoFlexiSingle();'));

					echo form()->button("ButtonUserSave", "button save", lang('Save Prod. Info Flexi'), array('click' => 'Ext.DOM.getVerificationStatus_2nd(Ext.DOM.Fallback);'));
				}?>
			</div>
		</div>
		<div class="ui-widget-form-row" >
