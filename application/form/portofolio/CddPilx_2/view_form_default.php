...
// 		PARAM['InputId'] 	= id;
// 		console.log('asd', Ext.EventUrl([Controller,'save_activity']).Apply());
// 		Ext.Ajax
// 		({
...
Ext.DOM.CheckVer = function(id)
{
	...
	if(id=="ask_1"){
		if(additional_cc=="" || additional_cc==null){
			...
			if (total_additional_cc == '' || addEventListener == null) {
	...
	if(readycheck == 1){
		...
		Ext.Ajax
		({
			url 	: Ext.EventUrl([Controller,'save_cc_additional']).Apply(),
			method : 'POST',
			param 	: Ext.Join(new Array
				( 
					Ext.Serialize('frm_verification').getElement(),
