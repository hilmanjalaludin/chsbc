 ...
 * @ nmae  : R_CallTrackAtmGroupByCampaign 
 * ----------------------------------------------------
 * @ param : - 
 * @ param : 
 */
...
class R_CallTrackAtmGroupByCampaign extends EUI_Report 
{
...
/* @ param : aksesor **/
 ...
 public function R_CallTrackAtmGroupByCampaign() 
{
...
public function _getResultReport() 
{
		...
		AND ($this -> Interval=='summary') )
	{
		$_conds = array ( 
			...
			'CALL_VIEWER' => 'call_track_summary_atm_groupby_campaign',
...
public function _getUserReport()
{
	...
	$this -> db -> where_in('a.UserId', $this -> AtmId);
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		$User[$UserId] = $rows;
	}
...
public function _getSummaryReport()
{
 ...
 /** ################################################################# get new data and data size ################################################################# **/
	...
	$this ->db ->select("
		COUNT(a.CustomerId) as JUMLAH,  a.CampaignId, a.CallReasonId, b.AssignSpv,
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		$_conds[$rows['AssignSpv']][$rows['CampaignId']]['data_size']+= (INT)$rows['JUMLAH'];
		$_conds[$rows['AssignSpv']][$rows['CampaignId']]['data_not_utilize']+= (INT)$rows['NewData'];
	}
 ...
 /** ################################################################# get callreason ID size ################################################################### **/
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		$_conds[$rows['AssignSpv']][$rows['CampaignId']][$rows['CallReasonId']]+= (INT)$rows['JUMLAH'];
		$_conds[$rows['AssignSpv']][$rows['CampaignId']]['data_utilize']+= (INT)$rows['Utilize'];
	}
	
 /** ########################################################## get size call atempt ########################################################################## **/

	$this->db->select("count(a.CallHistoryId) as size_atempt, b.CampaignId, c.AssignSpv",FALSE);
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		$_conds[$rows['AssignSpv']][$rows['CampaignId']]['size_atempt']+= (INT)$rows['size_atempt'];
	}	
...
/** ########################################################## hitung jumlah sales dan Premi total ############################################################## **/
   ...
   $this ->db->select("COUNT(c.PolicyId) as size_policy, 
				...
				b.CampaignId, f.AssignSpv",FALSE);
