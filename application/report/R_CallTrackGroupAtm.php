...
class R_CallTrackGroupAtm extends EUI_Report 
{
...
/* @ param : aksesor **/
 ...
 public function R_CallTrackGroupAtm() 
{
...
public function _getResultReport() 
{
		...
		AND ($this -> Interval=='summary') )
	{
		$_conds = array ( 
			...
			'CALL_VIEWER' => 'call_track_summary_by_atm',
...
public function _getSummaryReport()
{
 ...
 /** ################################################################# get new data and data size ################################################################# **/
	...
	$this ->db ->select("
		COUNT(a.CustomerId) as JUMLAH,  b.AssignSpv, a.CallReasonId, 
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		$_conds[$rows['AssignSpv']]['data_size']+= (INT)$rows['JUMLAH'];
		$_conds[$rows['AssignSpv']]['data_not_utilize']+= (INT)$rows['NewData'];
	}
 ...
 /** ################################################################# get callreason ID size ################################################################### **/
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		$_conds[$rows['AssignSpv']][$rows['CallReasonId']]+= (INT)$rows['JUMLAH'];
		$_conds[$rows['AssignSpv']]['data_utilize']+= (INT)$rows['Utilize'];
	}
	
 /** ########################################################## get size call atempt ########################################################################## **/

	$this->db->select("count(a.CallHistoryId) as size_atempt, c.AssignSpv",FALSE);
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		$_conds[$rows['AssignSpv']]['size_atempt']+= (INT)$rows['size_atempt'];
	}	
...
/** ########################################################## get name of the campaign ######################################################################### **/
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows ){
		$_conds[$rows['UserId']]['AtmName'] = $rows['full_name'];
	}	
...
/** ########################################################## hitung jumlah sales dan Premi total ############################################################## **/
   ...
   $this ->db->select("COUNT(c.PolicyId) as size_policy, 
	...
	foreach( $this -> db -> get() -> result_assoc() as $rows )
	{
		...
		{
			$_conds[$rows['AssignSpv']]['size_premi'] += $rows['size_premi'];
			$_conds[$rows['AssignSpv']]['size_policy']+= $rows['size_policy'];
		}	
	}	
