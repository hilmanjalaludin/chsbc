...
class R_CallTrackCampaignGroupTsr extends EUI_Report 
{
...
// ----------- set title ---> 
 $this->arr_label["ReportName"] = "CALLTRACKING & SUMMARY PRODUCTION REPORT GROUP BY TMR";
 $this->arr_label["HeaderName"] = "TMR";
...
// ---------------- next process ------------------------------
	...
	{
		$this->arr_result = array  ( 
			'call_viewer'	=> 'view_calltrack_campaign_group_tsr_summary_html',
...
// --- ambil data secara keseluruhan ---------------------------------
	...
	$this->db->reset_select();
	$this->db->select("
		COUNT(a.CustomerId) AS tot_size, b.AssignSelerId, 
	...
	$this->db->where_in("b.AssignSelerId", $out->get_array_value('UserId'));
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$result[$rows['AssignSelerId']]['data_size'] +=$rows['tot'];
		$result[$rows['AssignSelerId']]['data_not_utilize'] +=$rows['tot_unutilize'];
		$result[$rows['AssignSelerId']]['data_utilize']	+=$rows['tot_utilize'];
		$result[$rows['AssignSelerId']]['tot_data_status_101'] +=$rows['tot_status_101'];
		$result[$rows['AssignSelerId']]['tot_data_status_102'] +=$rows['tot_status_102'];
		$result[$rows['AssignSelerId']]['tot_data_status_103'] +=$rows['tot_status_103'];
		$result[$rows['AssignSelerId']]['tot_data_status_201'] +=$rows['tot_status_201'];
		$result[$rows['AssignSelerId']]['tot_data_status_202'] +=$rows['tot_status_202'];
		$result[$rows['AssignSelerId']]['tot_data_status_203'] +=$rows['tot_status_203'];
		$result[$rows['AssignSelerId']]['tot_data_status_204'] +=$rows['tot_status_204'];
		$result[$rows['AssignSelerId']]['tot_data_status_205'] +=$rows['tot_status_205'];
		$result[$rows['AssignSelerId']]['tot_data_status_206'] +=$rows['tot_status_206'];
		$result[$rows['AssignSelerId']]['tot_data_status_207'] +=$rows['tot_status_207'];
		$result[$rows['AssignSelerId']]['tot_data_status_208'] +=$rows['tot_status_208'];
		$result[$rows['AssignSelerId']]['tot_data_status_209'] +=$rows['tot_status_209'];
		$result[$rows['AssignSelerId']]['tot_data_status_210'] +=$rows['tot_status_210'];
		$result[$rows['AssignSelerId']]['tot_data_status_211'] +=$rows['tot_status_211'];
		$result[$rows['AssignSelerId']]['tot_data_status_301'] +=$rows['tot_status_301'];
		$result[$rows['AssignSelerId']]['tot_data_status_302'] +=$rows['tot_status_302'];
		$result[$rows['AssignSelerId']]['tot_data_status_303'] +=$rows['tot_status_303'];
		$result[$rows['AssignSelerId']]['tot_data_status_304'] +=$rows['tot_status_304'];
		$result[$rows['AssignSelerId']]['tot_data_status_305'] +=$rows['tot_status_305'];
		$result[$rows['AssignSelerId']]['tot_data_status_306'] +=$rows['tot_status_306'];
		$result[$rows['AssignSelerId']]['tot_data_status_307'] +=$rows['tot_status_307'];
		$result[$rows['AssignSelerId']]['tot_data_status_308'] +=$rows['tot_status_308'];
		$result[$rows['AssignSelerId']]['tot_data_status_309'] +=$rows['tot_status_309'];
		$result[$rows['AssignSelerId']]['tot_data_status_310'] +=$rows['tot_status_310'];
		$result[$rows['AssignSelerId']]['tot_data_status_311'] +=$rows['tot_status_311'];
		$result[$rows['AssignSelerId']]['tot_data_status_312'] +=$rows['tot_status_312'];
		$result[$rows['AssignSelerId']]['tot_data_status_313'] +=$rows['tot_status_313'];
		$result[$rows['AssignSelerId']]['tot_data_status_314'] +=$rows['tot_status_314'];
		$result[$rows['AssignSelerId']]['tot_data_status_315'] +=$rows['tot_status_315'];
		$result[$rows['AssignSelerId']]['tot_data_status_316'] +=$rows['tot_status_316'];
		$result[$rows['AssignSelerId']]['tot_data_status_317'] +=$rows['tot_status_317'];
		$result[$rows['AssignSelerId']]['tot_data_status_318'] +=$rows['tot_status_318'];
		$result[$rows['AssignSelerId']]['tot_data_status_401'] +=$rows['tot_status_401'];
		$result[$rows['AssignSelerId']]['tot_data_status_402'] +=$rows['tot_status_402'];
		$result[$rows['AssignSelerId']]['tot_data_status_601'] +=$rows['tot_status_601'];
		$result[$rows['AssignSelerId']]['tot_data_status_602'] +=$rows['tot_status_602'];
		$result[$rows['AssignSelerId']]['tot_data_status_603'] +=$rows['tot_status_603'];

	}
// ---------- get data talktime --------------
	
	$this->db->reset_select();
	$this->db->select("c.AssignSelerId as AssignSelerId, SUM(a.duration) as tot_talk_time ", FALSE);
	...
	$this->db->where_in("c.AssignSelerId", $out->get_array_value('UserId'));
	...
	$this->db->group_by(array("AssignSelerId"));
	//$this->db->print_out();
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$result[$rows['AssignSelerId']]['data_tot_talk_time'] +=$rows['tot_talk_time'];
	}	
// ---------- ambil data follow yang benar2 fress ---
	...
	$this->db->where_in("a.AssignSelerId", $out->get_array_value('UserId'));
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$result[$rows['AssignSelerId']]['tot_new_assigned']+=$rows['tot'];
	}
...
// ---------- ambil data follow yang tidak fress dan di followup kembali ---
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$result[$rows['AssignSelerId']]['tot_re_assigned']+=$rows['tot'];
	}
...
// ----- ambil data atempt di call history calltype = 0 --
	...
	$this->db->where_in("b.CampaignId", $out->get_array_value('CampaignId')); 
	$this->db->where_in("a.CreatedById", $out->get_array_value('UserId'));
	...
	if( $rs->num_rows() > 0 ) 
		foreach( $rs->result_assoc() as $rows ) 
	{
		$result[$rows['AssignSelerId']]['tot_data_atempt']+=$rows['tot'];
	}
...
// ------------ data policy sales ----------------------------------------------
	...
	$this->db->where_in("e.AssignSelerId", $out->get_array_value('UserId'));
	...
	$this->db->group_by(array("PolicyNumber","AssignSelerId"));
	
	$rs = $this->db->get();
	if($rs->num_rows() > 0)  
		foreach( $rs-> result_assoc() as $rows )
	{
		$result[$rows['AssignSelerId']]['total_data_policy'] += $rows['tot'];
		$result[$rows['AssignSelerId']]['tot_data_insured'] += $rows['tot_data_insured'];
		$result[$rows['AssignSelerId']]['tot_data_premi'] += $rows['tot_data_premi'];
	}	
