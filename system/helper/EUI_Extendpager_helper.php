...
function set_event_row_checked( $func="" )
{	
	if( is_null($func) ) return false;
   	$this->arr_checkbox_func_checked = $func;
}

function set_type_row_checked( $type="" )
{	
	if( is_null($type) ) return false;
   	$this->type_checkbox = $type;
}
 ...
 public function select_pager_content()
{
	...
	if( $this->arr_checkbox_func_checked != "" ) {
		$on_change_checkbox = $this->arr_checkbox_func_checked;
	} else {
		$on_change_checkbox = "Ext.Cmp('$primary').oneChecked(this);";
	}
	...
	//---------------------- chekcbox argument ---------------> 
		...
		if( $this->arr_checkbox_func ){
			if( $this->arr_onechecked_func ){
				...
				if( $this->type_checkbox != "" ){
					//radio
					echo "<td class=\"content-first center\">". form()->{$this->type_checkbox}( $primary, NULL, $row->get_value($primary), array("change" => "$on_change_checkbox"))."</td>";
				} else {
					echo "<td class=\"content-first center\">". form()->checkbox( $primary, NULL, $row->get_value($primary), array("change" => "$on_change_checkbox"))."</td>";
				}
