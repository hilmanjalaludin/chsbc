	...
	NotActive : function(){
			...
			|| (Ext.Session('HandlingType').getSession() == Ext.DOM.LEVEL.USER_AGENT_OUTBOUND) )
		{
			for( var a in this.menu ) {
				var $id = new Array('#', this.menu[a].id).join('');  _SetTmpActiveMenu[$id] = $($id).attr('onclick');
 ...
 var Session = {
	...
	getStore : function(){
		if( _Cookies.length < 1 ){
			this.Cokie = ( Ext.Ajax ({url: Ext.DOM.INDEX +'/ActiveMenu/UserSession/', method: 'GET', param:{}}).json() );
			this.Usergroup = ( Ext.Ajax ({url: Ext.DOM.INDEX +'/ActiveMenu/UserPrivilege/', method: 'GET', param:{}}).json() );
			this.Config = ( Ext.Ajax ({url: Ext.DOM.INDEX +'/ActiveMenu/UserConfig/', method: 'GET', param:{}}).json() );
		}	
...
Core.prototype.ActiveBars = function( level ){
  var Bars = 
  {
	title:[] , menu:[], icon:[], option:[], USER_LEVEL : parseInt(level), Toolbars : {},
	fnToolbar :function(){
		 _Toolbars = Ext.Ajax ({url: Ext.DOM.INDEX +'/ActiveMenu/Toolbars/', method: 'GET', param:{}}).json();
	...
	Title :function(){
		try
		{
			...
			var arr_obj_title = new Array(), arr_obj_store = this.Toolbars.title, title = arr_obj_store.map( function(item){
	...
	Menu : function(){
		try
		{
			var arr_obj_event = new Array(), arr_obj_store = _Toolbars.event, event = arr_obj_store.map( function(item){
	...
	Icon : function(){
		try
		{
			var arr_obj_icon = new Array(), arr_obj_store = _Toolbars.icons, icons = arr_obj_store.map( function(item){
	...
	Option : function() {
		try
		{
			var arr_obj_option = new Array(), arr_obj_store = _Toolbars.option, arr_other  = {};
			 for( var obj in arr_obj_store )
			{
				if( arr_obj_store[obj].type == 'combo' )
				{
					var $arr_list = arr_obj_store[obj], $arr_nobj = {}, arr_select = {};
...
if( typeof( window.Helper ) !== 'function' ){
	window.Helper = function(){ Ext.Knowledge() };
}
