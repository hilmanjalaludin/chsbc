...
{	
		...
		switch(callstatus) 
		{
			...
			case CALLSTATUS_ALERTING:
					...
					{	
						...
						// try { Ext.CallerScript(); } catch( e){ console.log( e ); };
			...
			if( v_status_agent!='') 
			{
				...
				if( Ext.Cmp('auxReason').empty()!=true ){
					try{ 
						...
						Ext.Error({log: e, name : 'CTI.agentSetNotReady()', lineNumber : e.lineNumber }); 
					}	
				}
				else{
	...
	setLabelReady:function(){
			try {
				...
				Ext.Error({log: e, name : 'CTI.setLabelReady()', lineNumber : e.lineNumber }); 
			}
		},
