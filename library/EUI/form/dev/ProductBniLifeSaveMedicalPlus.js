...
/* 
 * @ pakage code  		 0b009
 * @ package 		 	 Product BNI Life Save Medical Plus Java Script Language
...
function SelectValidationGroup(){
 var PremiTypeId = Ext.Cmp('InsuredPremiType').getValue();
 var GroupPremi = Ext.Cmp("GroupPremi").getCheckBox();
 if( PremiTypeId == 2 && GroupPremi.IsChecked().length < 2 ){
	return false;	
 } else {
	 return true;
 } 
...
function NodeClass( obj )
{
 ...
 if( $(obj).val() == 'Y'){
	$('.node-'+ classList[1]).attr('disabled', false);
	$('.node-'+ classList[1]).addClass('yes-required-input');
	$('.node-'+ classList[1]).removeClass('no-required-input');
	$('.node-'+ classList[1]).css({"border": "1px solid #FF4321"});
	
	
 } else {
	$('.node-'+ classList[1]).addClass('no-required-input');
	$('.node-'+ classList[1]).removeClass('yes-required-input');
	$('.node-'+ classList[1]).attr('disabled', true);
	$('.node-'+ classList[1]).css({"border": "1px solid #dddddd"});
 }
...
function UnderwriteIsSelected(){
	$(".ui-event-onload").each(function(){
		new NodeClass($(this));
	});
} 
// --------------------------------------------------------------------------------------------------------
 ...
 function SetEventSubmit() 
{
	var frmDataProduct = Ext.Serialize('frmDataProduct'),
	   frmPremiType	 = Ext.Serialize('frmPremiType'),
  ...
  var TotalPremiPayment	 = Ext.Cmp("DiscountAfterPremi").getValue();  
  ...
  if( !frmPremiType.Complete() ){
	Ext.Msg("Form Main Insured Not Complete").Info();
	new SelectedInterfaceTab(1);
	return false;	  
  }
  
  if( !frmMainInsured.Complete(new Array('SubmitedPremi_2_1','InsuredLastName_2_1')) ){
	Ext.Msg("Form Main Insured Not Complete").Info();
	new SelectedInterfaceTab(1);
	return false;	
  }
  
  if( !SelectValidationGroup() ){
	Ext.Msg("Please Select an Individual Premiums").Info();
	new SelectedInterfaceTab(1);
	return false;	
  }	
  ...
  if(!SelectValidationBenefiecery() ){
 ...
 // ------------- save data policy ---> 
  ...
  Ext.Ajax   ({
		url 	: Ext.EventUrl(new Array('ProductBniLifeSaveMedicalPlus','SaveDataEntry')).Apply(),
		method 	: 'POST',
...
{
	var personal_premi = Ext.Cmp("InsuredPremiType").getValue();
	var object = object, selector = 
	{
		listener : function() {
			return ( Ext.Ajax  ({
				url 	: Ext.EventUrl(new Array('ProductBniLifeSaveMedicalPlus','SelectPremiPersonal')).Apply(),
				method 	: 'POST',
				param 	: { 
					personal_data  : JSON.stringify(object),
					personal_premi : personal_premi 	
				}
			}).json() );
		},
 ...
 function MainInsuredCopy( cond )
{
 ...
 if( cond == VAR_MAIN_CODE )
 {
	var InsuredDOB = new Array('InsuredDOB',cond).join('_'),
		...
		InsuredLastName = new Array('InsuredLastName',cond).join('_'),
	...
	// --------------- one by one  -------------------------------------------	
		if( typeof (out[InsuredLastName]) == 'undefined') {
			if( !Ext.Cmp('PayerLastName').IsNull() ){
				Ext.Cmp(InsuredLastName).setValue( Ext.Cmp('PayerLastName').getValue() );
			} 
		}	
		else {
			if( !Ext.Cmp(InsuredLastName).IsNull() ){
				Ext.Cmp(InsuredLastName).setValue(out[InsuredLastName]);
			}
		}
...
{
	return( Ext.Ajax ({
			url 	: Ext.EventUrl(new Array('ProductBniLifeSaveMedicalPlus', 'SelectInsuredAge') ).Apply(), // +"/ProductForm/DOB/",
			method 	:'GET',
			param 	: { 
				Date  :  object.Date, 
...
{
	return( Ext.Ajax  ({
			url 	: Ext.EventUrl(new Array('ProductBniLifeSaveMedicalPlus', 'SelectCalculateAge') ).Apply(), 
			method 	:'GET',
			param 	: { 
 ...
 */
 ...
 {
	if( Ext.Cmp("InsuredPremiType").empty() ){
		Ext.Msg("Please select Premi Type").Info();
		return false;
	}
			...
			if( typeof( date ) =='string') 
			{
				...
				else
				{
					...
					if( typeof (data) == 'object' && data.success  == 1 ){ 
						...
						if( selector[1] == INT_DEPEND_CODE ){
							Ext.Msg('Invalid Age').Info();
...
function SetVisiblePremiGroup( object ){

 if( Ext.Cmp('InsuredPremiType').getValue() == 1 ) {
	SelectGroup  = {};	
	var arr_select = Ext.Cmp("GroupPremi").getValue();
	for( var i in arr_select ){
		if( arr_select[i] != '2_1' ){
			SelectGroup[arr_select[i]] = arr_select[i];
			$(this).attr("checked", false);
			$(this).attr("disabled", true);
		}
	}
	
	$(".ui-class-widget-box").each(function(){
		if( $(this).attr("value")!='2_1' ){
			$(this).attr("checked", false);
			$(this).attr("disabled", true);
			
			var selector = new Array(".form", $(this).attr('value')).join('_');
				$( selector ).each(function(){
					var $field = $(this).attr('id');
					tmp_selected_box[$field] =  Ext.Cmp($field).getValue('');
					Ext.Cmp($field).setValue('');
...
// =======================================================================
 ...
 if( Ext.Cmp('InsuredPremiType').getValue() == 2 
	&&  Object.keys(SelectGroup).length ==0 )
{
	...
	var arr_list = Object.keys(SelectGroup);
	$(".ui-class-widget-box").each(function(){
		
		if( $(this).attr("value")!='2_1' ){
			var find = $(this).attr('value');
			if( Ext.Array(arr_list).in_array(find) ){
				...
				$( selector ).each(function() {
					var $field = $(this).attr('id');
						Ext.Cmp($field).disabled(false);
						if( $(this).attr('value') != VAR_MAIN_CODE && typeof(tmp_selected_box[$field]) != 'undefined' )  {
							Ext.Cmp($field).setValue( tmp_selected_box[$field] );
						}
				});	
...
// ---------- set to server request  ---------------------------------------
	var total_premi = 0;
	var total_discount_premi = 0;
	var total_affter_discount_premi = 0;
	...
	if( typeof GroupPremiObject  == 'object') 
	{
			...
			for( var n in PersonalObject )
		{
			if( typeof PersonalObject[n].PremiPerson != 'undefined' )
			{
				...
				console.log($SubmitedIds);
				var $SubmitedIdr = Ext.Format(parseInt(PersonalObject[n].PremiAfterDiscount)).IDR();
					...
					total_discount_premi+=parseInt(PersonalObject[n].PremiDiscount);
					total_affter_discount_premi+=parseInt(PersonalObject[n].PremiAfterDiscount);
			}	
		}
	}	
...
// ------------ testing data ---------------------------
	...
	Ext.Cmp('DiscountPremi').disabled(true);
	Ext.Cmp('DiscountPremi').setValue( Ext.Format(total_discount_premi).IDR());
	Ext.Cmp('DiscountAfterPremi').disabled(true);
	Ext.Cmp('DiscountAfterPremi').setValue( Ext.Format(total_affter_discount_premi).IDR());
...
function WindowEvent() {
   ...
   $('#AddressKeywords').bind('keyup', function( event ){ 
		if( event.keyCode == 13 ) {
			SearchByKeyword({ orderby :'', type : '' }); 
			return false;
		} else {
			SearchByKeyword({ orderby :'', type : '' }); 
		}
 ...
 */
  ...
  $(document).ready(function() 
 {
	...
	new Protected();
	new UnderwriteIsSelected();
	new SetVisiblePremiGroup();
 });
