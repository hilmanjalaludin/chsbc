 ...
 * 
 */
//console.log("page :"+ $.cookie('selected'));
		...
		if (!$( '.aqPaging',this ).length ) 
		{
			...
			$('<div class="aqPaging" id="aqPaging_'
				+$.fn.aqPaging.defaults.uniqID+'"><\/div> &nbsp;'+( opts.rec ?''+
					...
					'<li class="middle page-bottom-custom-grid ui-widget-load-pager" style="border-right:0px solid #000;"></li>'+
					'<li class="middle total-record page-bottom-custom-grid" >Total Records : ( '+opts.records+' ) </li>'+
					''+( (typeof(opts.summary)=='undefined' || opts.summary=='') ? '' : '<li class="middle summary page-bottom-custom-grid">'+opts.summary+'</li>') +''+
					'<li class="lasted refresh page-bottom-custom-grid" '+ (typeof(opts.callfunc)=='object' ? 'onclick="$.Refresh();"' : '' ) +'>&nbsp;Refresh</li>'+
					+'</div>' : '') +'').appendTo(this);
